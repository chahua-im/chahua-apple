...
final class MediaHostingTests: XCTestCase {
    ...
    func testStickerEnteringViewportLoadsWithoutReconfiguration() async throws {
        ...
        let surface = StickerMediaSurfaceView(frame: CGRect(x: 20, y: 1200, width: 72, height: 72))
        scroll.addSubview(surface)
        surface.configure(
            media: .init(id: "scrolling-sticker", url: fixture.url.absoluteString,
        ...
        let renderer = try XCTUnwrap(surface.subviews.compactMap { $0 as? TimelineImageView }.first)
        let spinner = try XCTUnwrap(renderer.subviews.compactMap { $0 as? UIActivityIndicatorView }.first)
        XCTAssertTrue(spinner.isAnimating, "A visible pending sticker must not be blank without loading chrome")
        let available = expectation(description: "The visible sticker draws its downloaded pixels")
        ...
        let image = UIGraphicsImageRenderer(bounds: renderer.bounds).image { renderer.layer.render(in: $0.cgContext) }
        let pixels = try XCTUnwrap(image.cgImage)
        var rgba = [UInt8](repeating: 0, count: 4)
        try rgba.withUnsafeMutableBytes { buffer in
            let bitmap = try XCTUnwrap(CGContext(data: buffer.baseAddress, width: 1, height: 1, bitsPerComponent: 8,
