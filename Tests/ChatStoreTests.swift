...
final class ChatStoreTests: XCTestCase {
    ...
    func testDraftSubmissionIsDurableAndSharedAcrossStoreRestoration() async throws {
        ...
        await store.flushDraft(chatID: "chat")
        let submitted = await store.submitDraft(chatID: "chat")
        XCTAssertTrue(submitted)
        XCTAssertEqual(store.draftText(chatID: "chat"), "")
        XCTAssertEqual(queue.pendingMessages(chatID: "chat").map(\.text), ["hello\n世界"])
        store.setDraftText("unsent draft", chatID: "other")
        await store.flushDraft(chatID: "other")
        await queue.deactivate()
        store.reset()
        await queue.activate(uid: 1)
        XCTAssertEqual(store.draftText(chatID: "other"), "unsent draft")
        XCTAssertEqual(store.draftText(chatID: "chat"), "")
    ...
    func testTypingBeforeStorageRestorationIsNotOverwritten() async throws {
        ...
        await queue.deactivate()
        store.reset()
        store.setDraftText("new typing", chatID: "chat")
        await queue.activate(uid: 1)
        XCTAssertEqual(store.draftText(chatID: "chat"), "new typing")
        await store.flushDraft(chatID: "chat")
        await queue.deactivate()
        store.reset()
        await queue.activate(uid: 1)
    ...
    func testCompositionCancelsDebouncedSaveAndResumesWhenCommittedTextIsUnchanged() async throws {
        ...
        h.store.setDraftText("already committed", chatID: "chat")
        await Task.yield()
        h.store.setDraftComposing(true, chatID: "chat")
        ...
        h.store.setDraftText("already committed", chatID: "chat")
        h.store.setDraftComposing(false, chatID: "chat")
    ...
    func testCompositionDefersExplicitBackgroundAndRetryFlushesAndRefusesSubmission() async throws {
        let h = try await openDraftHarness()
        h.store.setDraftText("last persisted", chatID: "chat")
        await h.store.flushDraft(chatID: "chat")
        h.probe.draftWriteAttempts = 0
        h.probe.failDraftWrites = true
        h.store.setDraftComposing(true, chatID: "chat")
        h.store.setDraftText("final committed text", chatID: "chat")

        await h.store.flushDraft(chatID: "chat")
        await h.store.retryLocalStorage()
        h.store.setForegroundActive(false)
        let submitted = await h.store.submitDraft(chatID: "chat")
        try await Task.sleep(for: .milliseconds(650))

        XCTAssertFalse(submitted)
        XCTAssertEqual(h.probe.draftWriteAttempts, 0)
        XCTAssertEqual(h.probe.enqueueAttempts, 0)
        XCTAssertFalse(h.store.draftSaveFailed)
    ...
    func testCompositionCommitAutosavesFinalTextWithoutExplicitFlush() async throws {
        let h = try await openDraftHarness()
        h.store.setDraftText("previous", chatID: "chat")
        await h.store.flushDraft(chatID: "chat")
        h.probe.draftWriteAttempts = 0
        h.store.setDraftComposing(true, chatID: "chat")
        h.store.setDraftText("你好，世界", chatID: "chat")
    ...
    func testResetClearsCompositionAndCancelsDeferredSaveAcrossSessionRestoration() async throws {
        let h = try await openDraftHarness()
        h.store.setDraftComposing(true, chatID: "chat")
        h.store.setDraftText("discarded session", chatID: "chat")
        await h.store.flushDraft(chatID: "chat")
        h.store.setDraftComposing(false, chatID: "chat")
        h.store.reset()
        ...
        h.store.setDraftComposing(true, chatID: "chat")
        h.store.setDraftText("another discarded session", chatID: "chat")
        await h.store.flushDraft(chatID: "chat")
        h.store.reset()
        h.store.setDraftText("replacement session", chatID: "chat")
    ...
    func testFailedLocalEnqueueKeepsDraftAndRetryEnqueuesExactlyOnce() async throws {
        let h = try await openDraftHarness()
        h.store.setDraftText("keep this", chatID: "chat")
        await h.store.flushDraft(chatID: "chat")
        h.probe.failEnqueue = true
        let failed = await h.store.submitDraft(chatID: "chat")
        XCTAssertFalse(failed)
        XCTAssertEqual(h.store.draftText(chatID: "chat"), "keep this")
        XCTAssertTrue(h.store.draftSaveFailed)
        XCTAssertFalse(h.store.committingDrafts.contains("chat"))
        ...
        await h.store.retryLocalStorage()
        let retried = await h.store.submitDraft(chatID: "chat")
        XCTAssertTrue(retried)
        XCTAssertEqual(h.store.draftText(chatID: "chat"), "")
        XCTAssertFalse(h.store.draftSaveFailed)
    ...
    func testHistoryIngressAcknowledgesDurablePendingBeforeReturningSnapshot() async throws {
        ...
        let store = ChatStore(apiClient: api, outgoingQueue: queue, onInvalidToken: {})
        await queue.activate(uid: 1)
        store.setDraftText("delivered before response", chatID: "chat")
        let submitted = await store.submitDraft(chatID: "chat")
        XCTAssertTrue(submitted)
