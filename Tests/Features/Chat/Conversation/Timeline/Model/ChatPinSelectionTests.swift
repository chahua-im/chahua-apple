...
final class ChatPinSelectionTests: XCTestCase {
    func testViewportSelectsNearestOlderPinIncludingExactBoundary() throws {
        ...
        XCTAssertEqual(
            ChatPinSelection.activePin(in: pins, bottomVisibleMessageDate: nil)?.id, "pin-30")
        XCTAssertEqual(
            ChatPinSelection.activePin(
                in: pins, bottomVisibleMessageDate: pins[1].message.createdAt)?.id, "pin-20")
        XCTAssertEqual(
            ChatPinSelection.activePin(
                in: pins, bottomVisibleMessageDate: pins[1].message.createdAt.addingTimeInterval(-1)
            )?.id, "pin-10")
        XCTAssertEqual(
            ChatPinSelection.activePin(
                in: pins, bottomVisibleMessageDate: pins[2].message.createdAt.addingTimeInterval(-1)
            )?.id, "pin-10")
    }
}
