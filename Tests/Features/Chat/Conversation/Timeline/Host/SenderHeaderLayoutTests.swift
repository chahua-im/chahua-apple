...
final class SenderHeaderLayoutTests: XCTestCase {
        ...
        func testOutgoingSenderNameRendersWhiteInBothAppearances() throws {
            ...
            let presentation = try presentation(
                name: "Wei", gender: 1, outgoing: true, environment: environment)
            ...
            for scheme in [ColorScheme.light, .dark] {
                ...
                native.bind(
                    .init(
                        ...
                        actions: .init(), mediaContext: nil))
                native.layoutSubtreeIfNeeded()
                func senderLabel(in view: NSView) -> NSTextField? {
                    if let label = view as? NSTextField, label.stringValue == title.name {
                        return label
                    }
                ...
                XCTAssertGreaterThan(
                    whiteInk, 0, "The outgoing name must have visible white glyphs.")
        ...
        func testMeasuredHeaderRendersSameGlyphsAsUnconstrainedLabels() throws {
            ...
            let presentation = try presentation(
                name: "frogeater", gender: 1, environment: environment)
            let layout = TimelineLayoutEngine().layout(presentation, environment: environment)
            let native = TimelineBubbleContentView(
                frame: CGRect(origin: .zero, size: try XCTUnwrap(layout.frames[.bubble]).size))
            native.bind(
    ...
    func testQueuedSenderUsesLiveProfileUntilAcknowledgement() throws {
        ...
        func make(_ entry: ConversationTimelineEntry, profile: MeResponse?)
            -> TimelineRowPresentation
        {
            TimelineRowPresentation.make(
                row: .message(
                    .init(
                        entry: entry, isOutgoing: true, groupPosition: .single,
                        showsSenderName: true)),
                currentUserProfile: profile, currentUserID: 1, isThreadTimeline: false,
        ...
        let acknowledgement = try TimelineTestFixtures.message(
            id: "confirmed-sender", senderID: 1, at: 0,
        ...
        XCTAssertEqual(
            confirmed.name, "User 1",
            "An empty authoritative name must not resurrect the local profile name.")
    ...
    func testLegacyProfileWithoutGroupDoesNotStyleAnotherSender() throws {
        ...
        for senderID: Int32 in [1, 2] {
            let pending = PendingOutgoingMessage(
                ...
                body: .init(
                    messageType: .text, clientGeneratedId: "queued-\(senderID)", message: "Hi"),
            ...
            let presentation = TimelineRowPresentation.make(
                row: .message(
                    .init(
                        entry: .pending(pending), isOutgoing: true, groupPosition: .single,
                        showsSenderName: true)),
                currentUserProfile: profile, currentUserID: 1, isThreadTimeline: false,
                environment: environment
            )
    ...
    func testUnconstrainedHeaderFitsFullNameGroupAndGender() throws {
        ...
        for (name, gender, captionSize, expectedGlyph) in [
            ("wei", 0, CGFloat(12), "♂"), ("Wei", 1, CGFloat(12), "♂"),
            ("Wei Zhang", 2, CGFloat(17), "♀"),
        ] {
            for scale: CGFloat in [1, 2, 3] {
                ...
                let presentation = try presentation(
                    name: name, gender: gender, environment: environment)
                let title = try XCTUnwrap(presentation.title)
                ...
                XCTAssertLessThan(
                    frame.width, environment.centralWidth,
                    "This case must have room to display every label.")

                assertFits(
                    title.name, font: .systemFont(ofSize: captionSize, weight: .semibold),
                    in: items[0], scale: scale)
                assertFits(
                    try XCTUnwrap(title.groupName), font: .systemFont(ofSize: captionSize),
                    in: items[1], horizontalPadding: 10, scale: scale)
                assertFits(
                    try XCTUnwrap(title.genderGlyph), font: .systemFont(ofSize: captionSize),
                    in: items[2], scale: scale)
            }
        }
    }
    ...
    func testConstrainedHeaderPrioritizesNameOverGroupAndGender() throws {
        ...
        let presentation = try presentation(
            name: "Wei Zhang with a name longer than the available header", gender: 2,
            environment: environment
        )
        let title = try XCTUnwrap(presentation.title)
        let layout = TimelineLayoutEngine().layout(presentation, environment: environment)
        ...
        let nameSize = NSAttributedString(
            string: title.name,
            attributes: [
                .font: SenderHeaderFont.systemFont(
                    ofSize: environment.captionSize, weight: .semibold)
            ]
        ).size()
        XCTAssertGreaterThan(items[0].width, 0)
        XCTAssertLessThan(
            items[0].width, nameSize.width,
            "A constrained header must not force the long username to fit.")
    ...
    func testGenderFitsBeforeGroupWhenColumnCannotFitAllHeaderItems() throws {
        ...
        let title = try XCTUnwrap(presentation.title)
        assertFits(
            title.name, font: .systemFont(ofSize: environment.captionSize, weight: .semibold),
            in: layout.titleFrames[0], scale: environment.displayScale)
        assertFits(
            try XCTUnwrap(title.genderGlyph), font: .systemFont(ofSize: environment.captionSize),
    ...
    func testHeaderSharesBubbleInsetsAndContributesToBubbleHeight() throws {
        for outgoing in [false, true] {
            for width: CGFloat in [320, 900] {
                ...
                let presentation = try presentation(
                    name: "Wei Zhang with a longer username", gender: 1, outgoing: outgoing,
                    environment: environment
                )
                let layout = TimelineLayoutEngine().layout(presentation, environment: environment)
    ...
    private func presentation(
        ...
        let message = try TimelineTestFixtures.message(
            id: "sender-header", senderID: senderID, at: 0,
            fields: [
                "message": "Hi",
                "sender": [
    ...
    private func assertFits(
        ...
        XCTAssertGreaterThanOrEqual(
            frame.width + 0.0001, nativeSize.width + horizontalPadding,
            "\(text) must fit at display scale \(scale) without truncation or clipping.",
            file: file, line: line)
        XCTAssertGreaterThanOrEqual(
            frame.height + 0.0001, nativeSize.height,
    ...
    private func assertContainedNonoverlappingItems(
        ...
        for item in items {
            XCTAssertTrue(
                item.minX.isFinite && item.minY.isFinite && item.width.isFinite
                    && item.height.isFinite, file: file, line: line)
            ...
            XCTAssertTrue(
                ...
                "Every header label must remain inside the assigned title frame.", file: file,
                line: line)
        }
