...
final class SenderHeaderLayoutTests: XCTestCase {
    ...
    func testOutgoingSenderNameRendersWhiteInBothAppearances() throws {
        ...
        guard case .message(let row) = presentation.row else { return XCTFail("Missing message row") }
        ...
        for scheme in [ColorScheme.light, .dark] {
            let renderer = ImageRenderer(content:
                MessageSenderHeader(row: row, title: title, fontSize: environment.captionSize,
                                    size: frame.size, itemFrames: layout.titleFrames)
                    .background(Color.black)
                    .environment(\.colorScheme, scheme))
            renderer.scale = 2
            let bitmap = NSBitmapImageRep(cgImage: try XCTUnwrap(renderer.cgImage))
            let name = layout.titleFrames[0]
            var whiteInk = 0
            var coloredInk = 0
            for y in 0 ..< bitmap.pixelsHigh {
                for x in 0 ..< min(bitmap.pixelsWide, Int(name.maxX * renderer.scale)) {
    ...
    func testUnconstrainedHeaderFitsFullNameGroupAndGender() throws {
        ...
        for (name, gender, captionSize) in [("wei", 1, CGFloat(12)), ("Wei Zhang", 2, CGFloat(17))] {
    ...
    func testConstrainedHeaderTruncatesLongNameBeforeGroupAndGender() throws {
        ...
        assertFits(try XCTUnwrap(title.groupName), font: .systemFont(ofSize: environment.captionSize), in: items[1], horizontalPadding: 10, scale: environment.displayScale)
        assertFits(try XCTUnwrap(title.genderGlyph), font: .systemFont(ofSize: environment.captionSize), in: items[2], scale: environment.displayScale)
    }
    ...
    private func assertContainedNonoverlappingItems(
        ...
        for item in items {
            ...
            XCTAssertGreaterThan(item.width, 0, file: file, line: line)
