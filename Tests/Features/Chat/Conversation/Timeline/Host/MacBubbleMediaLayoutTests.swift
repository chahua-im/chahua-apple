...
@MainActor
final class MacBubbleMediaLayoutTests: XCTestCase {
    func testSmallImagesUpscaleWithoutExceedingTheAvailableLane() throws {
        let image = try attachment(id: "small", width: 40, height: 40)
        let size = try XCTUnwrap(MacBubbleMediaLayout.singleSize(for: image, viewport: CGSize(width: 600, height: 800), availableWidth: 300))
        XCTAssertEqual(size.width, 120, accuracy: 0.001)
        XCTAssertEqual(size.height, 120, accuracy: 0.001)
        let narrow = try XCTUnwrap(MacBubbleMediaLayout.singleSize(for: image, viewport: CGSize(width: 100, height: 80), availableWidth: 30))
    ...
    func testInvalidGalleryDimensionsUseTheSameFallbackAsMissingDimensions() throws {
        ...
        let expected = try XCTUnwrap(MacBubbleMediaLayout.gallery(for: missing, viewport: viewport, availableWidth: 300))
        let actual = try XCTUnwrap(MacBubbleMediaLayout.gallery(for: invalid, viewport: viewport, availableWidth: 300))
    ...
    func testAsymmetricGalleryChoosesBalancedRowsAndPreservesOrdering() throws {
        ...
        let gallery = try XCTUnwrap(MacBubbleMediaLayout.gallery(for: images, viewport: CGSize(width: 600, height: 400), availableWidth: 300))
    ...
    func testGalleryCapacityKeepsOverflowInTheSixthVisibleTile() throws {
        for count in [2, 3, 4, 5, 6, 7, 20] {
            ...
            let gallery = try XCTUnwrap(MacBubbleMediaLayout.gallery(
