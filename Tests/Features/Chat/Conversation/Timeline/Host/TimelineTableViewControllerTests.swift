    ...
    final class TimelineTableViewControllerTests: XCTestCase {
        func testOpeningWithFloatingHeaderLandsAtLatestMessage() async throws {
            ...
            let root = ChatSplitLayout(hasSelection: true) { _ in
                ...
                ConversationTimelineView(model: model, loadsInitialAutomatically: false)
                    .modifier(
                        ChatComposerOverlay {
                            MessageComposerView(
                                text: .constant(""), attachmentState: ComposerAttachmentState(),
                                maxHeight: 160,
            ...
            XCTAssertEqual(
                table.bounds.maxY, scroll.documentVisibleRect.maxY - scroll.contentInsets.bottom,
                accuracy: 1,
            ...
            let topGap =
                scroll.isFlipped ? track.minY - scroll.bounds.minY : scroll.bounds.maxY - track.maxY
            let bottomGap =
                scroll.isFlipped ? scroll.bounds.maxY - track.maxY : track.minY - scroll.bounds.minY
            XCTAssertEqual(
                topGap, scroll.contentInsets.top, accuracy: 1,
                "The indicator track must start below the header, without counting its inset twice."
            )
            XCTAssertEqual(
                bottomGap, scroll.contentInsets.bottom, accuracy: 1,
                "The indicator track must end at the composer, without counting its inset twice.")
            XCTAssertEqual(
                scroller.doubleValue, 1, accuracy: 0.001,
        ...
        func testUnreadMarkerAndReadTrackingRespectFloatingOverlays() async throws {
            ...
            let marker = try XCTUnwrap(
                table.view(atColumn: 0, row: markerIndex, makeIfNecessary: true)
                    as? TimelineTableCellView)
            marker.layoutSubtreeIfNeeded()
            XCTAssertEqual(
                table.rect(ofRow: markerIndex).minY,
            ...
            XCTAssertEqual(
                table.rect(ofRow: readBoundary).maxY,
            ...
            try await Task.sleep(for: .milliseconds(650))
            XCTAssertEqual(
                table.bounds.maxY, scroll.documentVisibleRect.maxY - controller.composerInset,
                accuracy: 1,
            ...
            func show(top: CGFloat, bottom: CGFloat) async throws {
                ...
                window.setContentSize(
                    NSSize(
                        width: 400,
                        height: bottom - top + controller.headerInset + controller.composerInset))
                controller.view.layoutSubtreeIfNeeded()
                controller.viewDidLayout()
                try await waitForDisplay(controller, model: model)
        ...
        func testLiveArrivalDuringInitialScrollCompletionIsRenderedWithoutAnotherLayout()
            async throws
        {
            ...
            let subscription = model.updates.sink { snapshot in
                guard !delivered, snapshot.pendingScroll == nil, !snapshot.rows.isEmpty else {
                    return
                }
                delivered = true
            ...
            XCTAssertEqual(
                table.numberOfRows, model.rows.count,
            ...
            for sender: Int32 in [1, 2] {
                for text in ["Hello", "Hi\nBye"] {
                    try await inspectRenderedMessage(
                        text: text, senderID: sender, widths: [320, 600, 900]
                        ...
                        XCTAssertGreaterThan(
                            bubbleWidth, 30, "The rendered bubble background must be present.")
                        XCTAssertLessThan(
                            bubbleWidth, 180,
                            "Short lines must not fill the maximum conversation lane.")
                    }
                }
            }
        }
        ...
        func testWrappedMessageGlyphsFitAfterNarrowingAndWideningTheTimeline() async throws {
            ...
            try await inspectRenderedMessage(text: text, senderID: 2, widths: [900, 320, 600]) {
                cell, textView, _ in
                self.assertGlyphsVisible(textView, in: cell)
            ...
            try await inspectRenderedMessage(
                ...
                enrich: { object in
                    ...
                    object["replyToMessage"] = [
                        ...
                        "messageType": "text",
                        "message": "A long quoted message "
                            + String(repeating: "that must truncate ", count: 10),
                    ...
                    object["attachments"] = (0..<7).map { index in
                        [
                            "id": "image-\(index)",
                            "url": "file:///chahua-test-missing-\(index).png", "kind": "image/png",
                            "size": 1, "fileName": "image.png", "width": 200 + index * 100,
                            "height": 300,
                        ] as [String: Any]
                    }
                },
        ...
        func testIncomingMetadataRemainsReadableInDarkAppearance() async throws {
            try await inspectRenderedMessage(
                text: "Hello", senderID: 2, widths: [320], appearance: .darkAqua
            ) { cell, textView, bitmap in
                guard let native = textView as? AppKitMessageTextView else {
                    return XCTFail("Missing native text view")
                }
                let frame = cell.convert(
                    native.contentLayout.geometry(for: native.bounds.width).metadataFrame,
                    from: native)
                ...
                for y in max(
                    0, Int(top * scaleY))..<min(
                        bitmap.pixelsHigh, Int(ceil((top + frame.height) * scaleY)))
                {
                    for x in max(
                        0, Int(frame.minX * scaleX))..<min(
                            bitmap.pixelsWide, Int(ceil(frame.maxX * scaleX)))
                    {
                ...
                XCTAssertGreaterThan(
                    brightest, 0.4,
                    "Timestamp ink must be visible against the dark incoming bubble.")
            }
        }

        func testFailedButtonsRemainClickableAcrossResizeAndAcknowledgement() async throws {
            let text =
                "Unsent text remains selectable.\n你好，世界 "
                + String(repeating: "Wrapped message text. ", count: 3)
            let pending = ["failed-a", "failed-b"].enumerated().map { index, id in
                PendingOutgoingMessage(
                    ...
                    enqueuedAt: TimelineTestFixtures.date(second: index), senderID: 1,
                    state: .failed
                )
            }
            let page = try TimelineTestFixtures.page([])
            ...
            func renderedMessage(_ id: String) throws -> (NSView, AppKitMessageTextView) {
                let index = try XCTUnwrap(
                    model.rows.firstIndex { $0.stableMessageKey == .clientGenerated(id) })
                table.scrollRowToVisible(index)
                let cell = try XCTUnwrap(table.view(atColumn: 0, row: index, makeIfNecessary: true))
                cell.layoutSubtreeIfNeeded()
                let native = try XCTUnwrap(
                    textViews(in: cell).compactMap { $0 as? AppKitMessageTextView }.first)
                XCTAssertEqual(
                    cell.bounds.height,
                    TimelineTestFixtures.layout(
                        row: model.rows[index], width: table.bounds.width, parent: controller,
                        cache: measurer
            ...
            for width: CGFloat in [320, 600, 900] {
                ...
                for item in pending {
                    ...
                    XCTAssertTrue(
                        native.accessibilityChildren()?.contains { ($0 as? NSButton) === button }
                            == true)
                    ...
                    XCTAssertTrue(
                        cell.bounds.insetBy(dx: -1, dy: -1).contains(
                            cell.convert(button.bounds, from: button)))
                    XCTAssertTrue(native.isSelectable)
                    native.setSelectedRange(NSRange(location: 0, length: 6))
                    XCTAssertEqual(
                        (native.string as NSString).substring(with: native.selectedRange()),
                        "Unsent")
                    assertGlyphsVisible(native, in: cell)
                    button.performClick(nil)
                    XCTAssertEqual(selected.last, item.clientGeneratedID)
                    let bitmap = try XCTUnwrap(
                        cell.bitmapImageRepForCachingDisplay(in: cell.bounds))
                    cell.cacheDisplay(in: cell.bounds, to: bitmap)
                    assertRedFailureSymbol(
                        in: bitmap, frame: cell.convert(button.bounds, from: button), cell: cell)
            ...
            XCTAssertEqual(
                selected, ["failed-a", "failed-b", "failed-a", "failed-b", "failed-a", "failed-b"])
            ...
            XCTAssertFalse(
                withoutAction.accessibilityChildren()?.contains { $0 is NSButton } == true)
            controller.actions = .init(openFailedMessage: { selected.append("updated:\($0)") })
            controller.view.layoutSubtreeIfNeeded()
            let (_, restoredAction) = try renderedMessage("failed-b")
            try XCTUnwrap(restoredAction.subviews.compactMap { $0 as? NSButton }.first)
                .performClick(nil)
            XCTAssertEqual(selected.last, "updated:failed-b")
        ...
        private func assertRedFailureSymbol(
            in bitmap: NSBitmapImageRep, frame: CGRect, cell: NSView,
            file: StaticString = #filePath, line: UInt = #line
        ) {
            let frame = frame.intersection(cell.bounds)
            guard !frame.isNull, !frame.isEmpty else {
                return XCTFail(
                    "The failure button must lie inside the rendered cell.", file: file, line: line)
            }
            let scaleX = CGFloat(bitmap.pixelsWide) / cell.bounds.width
            let scaleY = CGFloat(bitmap.pixelsHigh) / cell.bounds.height
            let top = cell.isFlipped ? frame.minY : cell.bounds.height - frame.maxY
            ...
            XCTAssertTrue(
                hasRedInk, "The failure button must render red ink inside its hit frame.",
                file: file, line: line)
        }
        ...
        private func inspectRenderedMessage(
            ...
            let base = try TimelineTestFixtures.message(
                id: "visible-text", senderID: senderID, at: 0, text: "Fixture")
            ...
            let controller = TimelineTableViewController(model: model)
            let window = NSWindow(
                contentRect: NSRect(x: 0, y: 0, width: widths[0], height: 1100),
                styleMask: [.titled], backing: .buffered, defer: false)
            window.isReleasedWhenClosed = false
        ...
        private func mountForDisplay(_ controller: TimelineTableViewController) -> NSWindow {
            let window = NSWindow(
                contentRect: controller.view.frame, styleMask: [.borderless], backing: .buffered,
                defer: false)
            window.isReleasedWhenClosed = false
            window.contentViewController = controller
            window.orderFront(nil)
        ...
        private func waitForDisplay(
            _ controller: TimelineTableViewController, model: ConversationTimelineModel,
            viewportIsSettled: () -> Bool = { true }
        ) async throws {
            let scroll = try XCTUnwrap(timelineScrollView(in: controller.view))
            let table = try XCTUnwrap(scroll.documentView as? NSTableView)
            ...
            repeat {
                ...
                let width =
                    scroll.contentView.bounds.width - scroll.contentInsets.left
                    - scroll.contentInsets.right
                if table.numberOfRows == model.rows.count, abs(table.bounds.width - width) < 0.5 {
                    ...
                    if visible.location != NSNotFound {
                        ...
                        let exact = indexes.allSatisfy { index in
                            let layout = TimelineTestFixtures.layout(
                                row: model.rows[index], width: width, parent: controller,
                                cache: cache)
                            guard abs(table.rect(ofRow: index).height - layout.size.height) < 0.5
                            else { return false }
                            if let cell = table.view(
                                atColumn: 0, row: index, makeIfNecessary: false)
                                as? TimelineTableCellView
                            {
                                return cell.rowView.binding?.presentation.row == model.rows[index]
                                    && cell.rowView.binding?.presentation.environment
                                        == TimelineTestFixtures.environment(
                                            width: width, parent: controller)
                            }
                            return true
                        }
            ...
            let details =
                ...
                ? "no visible rows"
                : (visible.location..<min(NSMaxRange(visible), model.rows.count)).map { index in
                    let expected = TimelineTestFixtures.layout(
                        row: model.rows[index], width: width, parent: controller, cache: cache)
                    let binding =
                        (table.view(atColumn: 0, row: index, makeIfNecessary: false)
                        as? TimelineTableCellView)?.rowView.binding
                    return
                        "\(index): height \(table.rect(ofRow: index).height)/\(expected.size.height), bound width \(binding?.presentation.environment.timelineWidth ?? -1), row matches \(binding?.presentation.row == model.rows[index])"
                }.joined(separator: "; ")
            XCTFail(
                "The display transaction did not install exact visible geometry. Width \(table.bounds.width)/\(width). \(details)"
            )
        }

        private func assertGlyphsVisible(
            _ textView: NSTextView, in cell: NSView, file: StaticString = #filePath,
            line: UInt = #line
        ) {
            guard let manager = textView.layoutManager, let container = textView.textContainer
            else {
                return XCTFail(
                    "Selectable text must have a laid-out text container.", file: file, line: line)
            }
            manager.ensureLayout(for: container)
            let glyphs = manager.glyphRange(for: container)
            XCTAssertEqual(
                NSMaxRange(manager.characterRange(forGlyphRange: glyphs, actualGlyphRange: nil)),
                (textView.string as NSString).length, "Every character must be laid out.",
                file: file, line: line)
            ...
            while let view = ancestor {
                if view === textView || view === cell || view.clipsToBounds {
                    ...
                    XCTAssertTrue(
                        view.bounds.insetBy(dx: -1, dy: -1).contains(converted),
                        "Glyphs \(converted) are clipped by \(type(of: view)) bounds \(view.bounds).",
                        file: file, line: line)
                }
                if view === cell { break }
                ...
                for x in 0..<bitmap.pixelsWide {
                    ...
                    let matches =
                        ...
                        ? color.blueComponent > color.redComponent + 0.2
                            && color.greenComponent > 0.3 && color.greenComponent < 0.7
        ...
        func testLiveResizeSettlesEveryRowAtFinalWidth() async throws {
            let page = try TimelineTestFixtures.page(
                (0..<40).map {
                    try TimelineTestFixtures.message(
                        ...
                        text: String(
                            repeating: "Wrapping text during interactive resizing. ", count: 4))
                })
            let model = ConversationTimelineModel(
            ...
            for width: CGFloat in [600, 900, 320] {
                ...
                for index in visible.location..<min(NSMaxRange(visible), model.rows.count) {
                    guard let cell = table.view(atColumn: 0, row: index, makeIfNecessary: true)
                    else { continue }
                    cell.layoutSubtreeIfNeeded()
                    let bitmap = try XCTUnwrap(
                        cell.bitmapImageRepForCachingDisplay(in: cell.bounds))
            ...
            let referenceModel = ConversationTimelineModel(
                chatID: "chat", currentUserID: 1, isGroupChat: true,
                source: HistorySource(initial: page, older: page),
                messageStore: ConversationMessageStore())
            ...
            let referenceScroll = try XCTUnwrap(
                reference.view.subviews.compactMap { $0 as? NSScrollView }.first)
            let referenceTable = try XCTUnwrap(referenceScroll.documentView as? NSTableView)
            try await waitForRowHeights(
                table, expected: model.rows.indices.map { referenceTable.rect(ofRow: $0).height }
            ) {
                XCTAssertEqual(
                    table.bounds.height - scroll.documentVisibleRect.maxY, 0, accuracy: 1)
            }
            for index in model.rows.indices {
                XCTAssertEqual(
                    table.rect(ofRow: index).height, referenceTable.rect(ofRow: index).height,
                    accuracy: 1,
        ...
        func testSplitResizeEnvironmentDefersOffscreenRowsAndPreservesHistoryAnchor() async throws {
            ...
            guard table.numberOfRows == model.rows.count, scroll.documentVisibleRect.height > 0
            else {
                XCTFail(
                    "The production conversation container must install rows in a nonzero viewport before resizing."
                )
                return
            }
            let measurer = TimelineLayoutCache()
            ...
            XCTAssertEqual(
                originalHeight,
                TimelineTestFixtures.layout(
                    row: model.rows[offscreen], width: table.bounds.width, parent: controller,
                    cache: measurer
                ).size.height,
                accuracy: 1)
            ...
            host.view.layoutSubtreeIfNeeded()
            XCTAssertTrue(
                controller.isSplitResizing,
                "SwiftUI must deliver the divider environment before its first width change.")
            for width: CGFloat in [620, 420] {
                ...
                XCTAssertEqual(
                    table.rect(ofRow: offscreen).height, originalHeight, accuracy: 1,
                    "Divider updates must not remeasure the offscreen history.")
                XCTAssertEqual(
                    table.rect(ofRow: anchor).minY - scroll.documentVisibleRect.minY, offset,
                    accuracy: 1)
                let visible = table.rows(in: scroll.documentVisibleRect)
                XCTAssertGreaterThan(visible.length, 0)
                for index in visible.location..<min(NSMaxRange(visible), model.rows.count) {
                    XCTAssertEqual(
                        table.rect(ofRow: index).height,
                        TimelineTestFixtures.layout(
                            row: model.rows[index], width: table.bounds.width, parent: controller,
                            cache: measurer
                        ).size.height,
            ...
            XCTAssertFalse(
                controller.isSplitResizing,
                "Ending the gesture must reach the same native controller.")
            XCTAssertEqual(
                table.rect(ofRow: offscreen).height, originalHeight, accuracy: 1,
                "Mouse-up must yield before remeasuring the offscreen history.")
            let expected = model.rows.map {
                TimelineTestFixtures.layout(
                    row: $0, width: table.bounds.width, parent: controller, cache: measurer
                ).size.height
            }
            try await waitForRowHeights(table, expected: expected) {
                XCTAssertEqual(
                    table.rect(ofRow: newAnchor).minY - scroll.documentVisibleRect.minY, newOffset,
                    accuracy: 1)
            }
            ...
            let page = try TimelineTestFixtures.page(
                (0..<40).map {
                    try TimelineTestFixtures.message(
                        id: "\($0)", senderID: 2, at: $0,
                        text: String(
                            repeating: "Wrapping text during overlapping resize interactions. ",
                            count: 5))
                })
            let model = ConversationTimelineModel(
                chatID: "chat", currentUserID: 1, isGroupChat: false,
        ...
        func testHeightSettlementSurvivesReentrantEditAndAnotherResize() async throws {
            let page = try TimelineTestFixtures.page(
                (0..<120).map {
                    try TimelineTestFixtures.message(
                        id: "\($0)", senderID: 2, at: $0,
                        text: String(
                            repeating: "Text that reflows across multiple settlement batches. ",
                            count: 6))
                })
            let store = ConversationMessageStore()
            let model = ConversationTimelineModel(
            ...
            let edited = try TimelineTestFixtures.message(
                id: "90", senderID: 2, at: 90,
                text: String(
                    repeating: "Edited while native height correction restores the reader. ",
                    count: 20))
            var delivered = false
            ...
            XCTAssertTrue(
                delivered,
                "Exercise a real reentrant edit while the settlement batch restores its anchor.")
            controller.isSplitResizing = true
            ...
            try await waitForRowHeights(table, expected: expected) {
                XCTAssertEqual(
                    table.rect(ofRow: anchor).minY - scroll.documentVisibleRect.minY, -12,
                    accuracy: 1)
            }
            XCTAssertFalse(model.state.live.followsLatest)
        }
        ...
        private func waitForRowHeights(
            ...
            let deadline = ProcessInfo.processInfo.systemUptime + 3
            while expected.indices.contains(where: {
                abs(table.rect(ofRow: $0).height - expected[$0]) > 1
            }) {
                checkViewport()
        ...
        func testResizingPreservesBottomAttachmentAndHistoryAnchor() async throws {
            let page = try TimelineTestFixtures.page(
                (0..<40).map {
                    try TimelineTestFixtures.message(
                        id: "\($0)", senderID: 2, at: $0,
                        text: String(
                            repeating: "Wrapping text changes the height of this message. ",
                            count: 4)
                    )
                })
            let model = ConversationTimelineModel(
            ...
            try await waitForDisplay(controller, model: model)
            XCTAssertEqual(
                table.rect(ofRow: index).minY - scroll.documentVisibleRect.minY, offset,
                accuracy: 1,
        ...
        func testUserScrollCancelsAnimatedJumpWithoutLaterMovement() async throws {
            ...
            try await Task.sleep(for: .milliseconds(350))
            XCTAssertEqual(
                scroll.documentVisibleRect.minY, userPosition, accuracy: 1,
        ...
        private func checkSameDayPrepend(scrollY: CGFloat) async throws {
            func page(_ ids: ClosedRange<Int>, olderCursor: String?) throws -> ListMessagesResponse
            {
                try TimelineTestFixtures.page(
                    ids.map {
                        try TimelineTestFixtures.message(
                            id: "\($0)", at: $0 % 60, hour: 12, minute: $0 / 60)
            ...
            let offsetAfter =
                tableView.rect(ofRow: newIndex).minY - scrollView.documentVisibleRect.minY
            XCTAssertEqual(
            ...
            XCTAssertEqual(
                source.queries.compactMap(\.before), ["60"],
        ...
        func testTimelineHasNoHorizontalScrollRangeAfterResizing() async throws {
            ...
            for width: CGFloat in [800, 400, 900, 300] {
                ...
                XCTAssertEqual(clip.constrainBoundsRect(proposed).origin.x, 0, accuracy: 0.5)
                XCTAssertGreaterThan(
                    tableView.frame.height, clip.bounds.height,
                    "vertical history scrolling remains available")
            }
        }
        ...
        private func checkHistoryScroll(legacyMouse: Bool) async throws {
            ...
            XCTAssertFalse(model.state.live.followsLatest)
            XCTAssertGreaterThan(
                scrollView.documentVisibleRect.minY, 0,
                "prepending history preserves the reader's position")
            XCTAssertGreaterThan(
                tableView.bounds.height - scrollView.documentVisibleRect.maxY, 400,
                "history loading must not jump to the bottom")
        }
    }
