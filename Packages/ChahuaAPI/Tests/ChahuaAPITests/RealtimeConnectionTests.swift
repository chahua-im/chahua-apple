...
final class RealtimeConnectionTests: XCTestCase {
        ...
        do {
            ...
            let client = ChahuaClient(
                configuration: ChahuaConfiguration(
                    baseURL: URL(
                        string: baseURL.absoluteString + "api/?inherited=secret#fragment")!,
            ...
            guard case .pong = try await connection.receive() else {
                return XCTFail("Expected first pong")
            }
    ...
    func testPresenceTransitionsAndHeartbeatsRetainAuthenticatedSocket() async throws {
        ...
        do {
            let client = ChahuaClient(
                configuration: ChahuaConfiguration(baseURL: await server.baseURL),
                token: "installed")
            let connection = try await client.openRealtimeConnection()
            ...
            for state in states {
                ...
                guard case .pong = try await connection.receive() else {
                    return XCTFail("Expected heartbeat pong")
                }
            }
    ...
    func testRefreshAndReconnectShareNewCredentialWithoutAdditionalRefresh() async throws {
        ...
        do {
            let client = ChahuaClient(
                configuration: ChahuaConfiguration(baseURL: await server.baseURL), token: "expired")
            ...
            let requests = await server.requests
            XCTAssertEqual(
                requests.map(\.path), ["/users/me", "/auth/refresh", "/users/me", "/ws", "/ws"])
    ...
    func testOldHeldRefreshCannotOverwriteNewAuthenticatedAccount() async throws {
        ...
        do {
            ...
            XCTAssertEqual(
                requests.last { $0.path == "/users/me" }?.headers["authorization"],
                "Bearer account-B")
            let auth = await server.authentications
    ...
    func testUnknownAndUnhandledFramesDoNotInterruptLaterEventsButMalformedKnownFrameFails()
        async throws
    {
        let server = try await RealtimeLoopbackServer.start()
        do {
            await server.setFrames([
                ...
                .string(
                    #"{"type":"message","payload":{"id":"9007199254740993","chatId":"42","clientGeneratedId":"c","messageType":"text","sender":{"uid":1,"gender":0},"createdAt":"2026-09-01T00:00:00Z","isEdited":false,"isDeleted":false,"hasAttachments":false,"attachments":[],"reactions":[],"message":"received"}}"#
                ),
            ...
            guard case .friendshipRemoved(let payload) = try await connection.receive() else {
                return XCTFail("Expected typed no-op")
            }
            XCTAssertEqual(payload.actorUid, 8)
            guard case .unknown(let type) = try await connection.receive() else {
                return XCTFail("Expected future event")
            }
            XCTAssertEqual(type, "futureFeature")
            guard case .message(let message) = try await connection.receive() else {
    ...
    private func accept(_ connection: NWConnection) async {
        ...
        do {
            ...
            if request.path == "/ws" || request.path == "/api/ws" {
                ...
                guard authOpcode == 1, auth.type == "auth", let ticket = auth.ticket else {
                    throw APIError.unexpectedResponse
                }
                ...
                while true {
                    ...
                    guard opcode == 1, frame.state == "active" || frame.state == "inactive",
                        frame.type == "appState" || frame.type == "ping"
                    ...
                    if !sentEvents {
                        ...
                        for event in frames {
                            ...
                            case .string(let text):
                                try await wire.sendFrame(opcode: 1, data: Data(text.utf8))
                            case .data(let data): try await wire.sendFrame(opcode: 2, data: data)
                ...
                if request.headers["authorization"] == "Bearer expired" {
                    ...
                    try await wire.respond(
                        status: 200,
                        body:
                            #"{"uid":1,"username":"fixture","gender":0,"stickerPackOrder":[],"permissions":[]}"#
                    )
                }
            } else {
...
private final class LoopbackWire: @unchecked Sendable {
    ...
    func readRequest() async throws -> RealtimeLoopbackServer.Request {
        ...
        guard let path = lines.first?.split(separator: " ").dropFirst().first else {
            throw APIError.unexpectedResponse
        }
    ...
    func sendFrame(opcode: UInt8, data: Data) async throws {
        ...
        if data.count < 126 {
            ...
            frame.append(contentsOf: [126, UInt8(data.count >> 8), UInt8(data.count & 0xff)])
        }
        frame.append(data)
