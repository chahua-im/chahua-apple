...
Settings has General (Language; Appearance for message text size and unread badge color; Conversations; Storage), Stickers (the two account-scoped auto-sort switches, Quick Reaction management, and Manage Sticker Packs), and Notifications (Push Notifications), followed by a separated Sign out action. The pack manager retains pack creation, reordering, and detail navigation; reaction pinning is independent of pack loading. iOS pushes the destinations in one `NavigationStack`, while macOS keeps categories in a split sidebar. Both settings navigation containers resolve their native “Settings” title from the observed language preference so it updates while the page is open; all section and destination labels have string-catalog translations.
