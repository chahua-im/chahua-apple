...
One connection is shared by all windows for the authenticated account, including when no scene is active. Aggregate per-window `scenePhase == .active`: at least one active scene advertises `active`; no active scenes advertises `inactive`. Losing focus, backgrounding, or closing the last window publishes presence without cancelling the receiver, heartbeat, or reconnect runner. Removing one of two active windows does not change aggregate presence.
...
4. `openRealtimeConnection()` returns after authentication only. The coordinator then sends `{"type":"appState","state":"active"}` or `{"type":"appState","state":"inactive"}`, reading current scene activity after the asynchronous opening completes. There is no implicit initial active ping. Initial/reconnect bulk HTTP recovery runs only while foreground-active and does not block receiving frames.
5. Send JSON `{"type":"ping","state":"<current state>"}` every 10 seconds, including while inactive. Preserve a 10-second outstanding-pong timeout; a simultaneous heartbeat must not replace an unanswered ping's deadline. Reconnect regardless of focus on close, transport/protocol failure, or timeout. Backoff starts at one second, doubles with 0–20% jitter, caps the complete delay at 30 seconds, and resets after five seconds on an open connection rather than on a focus transition.

Each aggregate activity transition promptly sends `{"type":"appState","state":"<current state>"}` on the retained socket. State frames and heartbeats are serialized without cancelling in-flight sends on focus changes. Inactive presence allows the server to send push again; it is not a request to disconnect. Foreground outbox, draft-flush, read-dwell, and bulk recovery gating remain separate from socket lifetime. Event-driven reductions and required hydration still run for incoming events. Do not use RFC WebSocket ping as the application heartbeat. `presenceUpdate.activeConnections` is a same-account registered-connection count, not another user's online status.

The presence/frame contract follows the PWA's `src/api/ws.ts` (`onopen`, `publishAppState`, 10-second ping interval, stable-connection reset/backoff) and `src/hooks/useAppLifecycle.ts` / `src/utils/dom.ts` (visible and focused means active; blur/visibility loss means inactive, not closed). The PWA also explicitly reconnects on browser online events and closes on offline events; Apple continues using transport failure/timeout instead of adding a separate network monitor. Apple retains its pong timeout and session-generation protections. iOS may suspend network/timer execution while backgrounded; this does not authorize intentional focus-loss closure or a background-execution entitlement, and resumed work uses the same timeout/reconnect path with current presence.
...
| Initial connection/reconnect while active, or inactive-to-active transition | Refresh chats, subscribed threads, and visible conversation windows | Receive and apply WS events concurrently with recovery; inactive presence alone never reconnects |
...
The shared segmented picker controls list membership, not selection identity. Messages always combines active group chats and DMs; the persisted “Show 'Threads' in Messages” setting determines whether it also includes subscribed active threads. The Threads scope remains available regardless of that setting. Active and archived Messages rows, unread badges, loading, and refresh use the same choice. Groups and DMs filter the active chats by kind. Lists sort by the later of server activity and local draft activity. Changing scope does not discard the selected conversation.

On iOS, the list's native navigation toolbar holds the account control and scope picker. Archive is a typed `NavigationStack` destination, not a replacement for the active root: the system owns push/pop animation, back controls, and interactive back navigation. On phones, the path retains the archive prefix through conversation and thread destinations. The iPad sidebar owns only the list path, so opening or leaving Archive preserves the selected detail. macOS keeps instantaneous list switching and its existing stacked `ConversationListHeader`. Archive initially inherits the active filter; subsequent changes affect only its own scope.

On macOS, `ChatSplitLayout` reserves a 52-point sidebar header beneath the transparent title bar. AppKit owns and positions the three standard window buttons; chat selection and navigation-title updates must not reparent them into SwiftUI or alter their position. The sidebar reservation keeps list controls clear of the buttons without tying window chrome to view identity.

Both modes use the same `ChatListView`, row, and swipe controls. Archived mode changes only the Archive action to Unarchive with the tray symbol; read/unread and mute/unmute keep the shared eligibility rules and handlers. The toolbar's shared scope picker has a flexible width bounded by the list viewport, leaving room for the leading control without overflowing the right edge or collapsing into the toolbar menu.

Settings has General (Language; Appearance for message text size and unread badge color; Conversations with a Stickers subsection for the two account-scoped auto-sort switches; Storage), Stickers (Quick Reaction management and Manage Sticker Packs), and Notifications (Push Notifications), followed by a separated Sign out action. Storage displays the account's cached-media size and offers Clear media cache as a separate destructive button row. The pack manager retains pack creation, reordering, and detail navigation; subscribed packs show their stickers as ordinary, non-interactive previews rather than disabled removal buttons. Reaction pinning is independent of pack loading. iOS pushes the destinations in one `NavigationStack`, while macOS keeps categories in a split sidebar. Both settings navigation containers resolve their native “Settings” title from the observed language preference so it updates while the page is open; all section and destination labels have string-catalog translations.

The scope picker uses the native segmented control's own appearance and interaction, including iOS 26's glass selection treatment. Do not wrap it in an additional interactive glass surface; its toolbar item disables the shared background on iOS 26. The active list shows the signed-in user's avatar before the picker; Archive uses the system back control in that position. Phone detail screens retain the native back button and use `ChatPhoneDetailHeader` for a separate avatar/title capsule. Adaptive iPad detail headers retain their separate identity and back/close controls.
...
The iOS and macOS main composer use one growing SwiftUI `TextField`, one `ComposerInputState`, and the same `ChatDraftStore`/`OutgoingMessageQueue` pipeline. The macOS attachment caption uses a native scrolling text view, bounded to one through six lines, with the same input-state and draft-ownership contract. `ChatDetailView` observes `ChatDraftStore` directly. `ChatStore` routes revision-filtered outgoing snapshots to it and coordinates reset, background flush, and storage retry. `MessageComposerView` owns layout and submission; its native bridge owns the one-shot desktop entry-focus intent. The editing session owns only transient input and composition boundaries. The external draft binding remains the committed authority—there is no second cached committed string or retained send callback.

`ComposerNativeInput.swift` isolates the native facts SwiftUI does not expose: marked text, editor ownership, editing-end snapshots, and undo/redo notifications. It observes SwiftUI's editor without installing a delegate or replacing editing, selection, or undo; only its owned native caption editor installs a text-change delegate. Desktop entry focus waits for an enabled, mounted editor in the key window, places the caret at the draft's end once, and is cancelled by intervening user input or detachment. Subsequent updates never reapply that initial selection policy. A temporary run-loop observer settles edits after native input transactions and detects unchanged-string unmark. Native teardown captures the final owned snapshot before deferring observable publication outside SwiftUI's update.
...
The shared `TimelineLayoutEngine` measures thread navigation as the final full-width bubble section, including its icon, reply label, divider, and trailing chevron. Its natural width contributes to bubble sizing so short messages do not unnecessarily truncate the reply count. The avatar anchors to the completed bubble bottom and reactions follow outside. Both platform renderers consume the footer's prepared rectangles; the macOS control lives inside `TimelineBubbleContentView`'s clipped sections. Otherwise-transparent sticker/media content stays transparent above the footer. Interaction previews retain the footer's appearance without enabling thread navigation.
...
`TextMessage/MessageTextContent` constructs attributed body text; `MessageTextLayout` owns TextKit wrapping, mention backgrounds, and inline metadata geometry. AppKit/UIKit text adapters retain native selection and link handling. Shared `MessageMetadata` supplies timestamp/delivery content and drawing; `MessageMetadataView` renders standalone media/sticker metadata without empty text storage. Inline metadata remains outside selectable body text. Captions under images use the same text geometry as text-only bubbles: the timestamp shares the last line when it fits and moves below the body within the text section otherwise. Image-only timestamps overlay the media. Headerless text starts with 8-point top and bottom insets around its measured text frame. A short final bubble splits any extra height needed to match its avatar between those insets; single-line text then shifts by the measured glyph-and-timestamp bounds so the visible content, rather than the line allocation, is centered. Without reactions or separators, adjacent bubbles have a 4-point gap within a sender run and an 8-point gap across run boundaries.
...
- [RealtimeCoordinator](../../App/Features/Chat/Shared/RealtimeCoordinator.swift): session socket, aggregate presence, stateful heartbeat, and foreground recovery lifecycle.
