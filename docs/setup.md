...
On macOS, the split layout extends into the hidden title-bar area. `SidebarWindowControls` hosts the window's actual traffic-light buttons in a dedicated row inside the sidebar, above Chats. It preserves native button actions, reapplies their sidebar positions after AppKit window resizing, and restores them to the system title bar in full screen or when the sidebar is removed. Leaving full screen embeds them again. iOS retains its existing safe-area layout.
