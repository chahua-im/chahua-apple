...
struct FixtureGalleryView: View {
    ...
    @ScaledMetric(relativeTo: .caption2) private var caption2Size = TimelineFixtureTypography
        .caption2
        ...
        NavigationStack {
            GeometryReader { geometry in
                List {
                    ...
                    Section {
                        ChahuaTextField(
                            title: "Username", prompt: "Username", text: .constant("fixture-user"))
                        ChahuaSecureField(
                            title: "Password", prompt: "Password", text: .constant(""),
                    ...
                    Section {
                        ...
                        ChahuaEmptyStateView(
                            title: "No content", message: "There is nothing to show yet.",
                            systemImage: "tray")
                        ChahuaRecoverableErrorView(
                            title: "Something went wrong",
                            message: "Try again when you are connected.", retryTitle: "Try again",
                            onRetry: {})
                    } header: {
    ...
    private func fixtureRow(_ row: TimelineRow, width: CGFloat) -> some View {
        ...
        let presentation = TimelineRowPresentation.make(
            row: row, currentUserProfile: nil, currentUserID: nil, isThreadTimeline: false,
...
#Preview("Simplified Chinese") {
    FixtureGalleryView().environment(\.locale, Locale(identifier: "zh-Hans"))
}
#Preview("Traditional Chinese") {
    FixtureGalleryView().environment(\.locale, Locale(identifier: "zh-Hant"))
}
...
#Preview("Accessible text bubbles") {
    FixtureGalleryView().environment(\.dynamicTypeSize, .accessibility3)
}
