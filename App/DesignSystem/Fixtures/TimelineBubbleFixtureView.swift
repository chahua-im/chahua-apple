    ...
    struct TimelineBubbleFixtureView: View {
                ...
                ChatSplitLayout(hasSelection: hasSelection) { _ in
                    VStack(alignment: .leading, spacing: 0) {
                        ConversationListHeader(selection: $listScope) {
                            Menu {
                                ...
                                AvatarView(
                                    url: ProcessInfo.processInfo.environment[
                                        "CHAHUA_FIXTURE_AVATAR"
                    ...
                    .modifier(
                        ChatHeaderOverlay(isVisible: hasSelection) {
                            ChatFloatingHeader(
                                title: "Native bubble timeline", onClose: { hasSelection = false }
                            ) {
                                AvatarView(
                                    url: nil, displayName: "Native bubble timeline", diameter: 32)
                            }
            ...
            VStack(spacing: 0) {
                ...
                if let model = fixture.timeline {
                        ...
                        ConversationTimelineView(
                            model: model,
                            initialPosition: ProcessInfo.processInfo.environment[
                                "CHAHUA_FIXTURE_MESSAGE"
                            ].map(TimelineInitialPosition.message) ?? .liveEdge,
                            actions: actions,
                            interactionContext: .init(
                                canWrite: handlersEnabled, isAdmin: true, isThreadView: threadScope)
                        )
        ...
        private var diagnosticControls: some View {
            VStack(alignment: .leading, spacing: 8) {
                ...
                HStack {
                    ...
                    Button {
                        fixture.store.markSending(
                            chatID: "bubble-fixtures", clientGeneratedID: "diagnostic-pending")
                    } label: {
                ...
                Button {
                    ...
                    Button {
                        if let current = scrollExperiment {
                            ...
                            startScrollExperiment()
                        }
                    } label: {
                        Text(
                            verbatim: scrollExperiment == nil
                                ? "Run scroll sweep (1,200 frames)" : "Stop scroll sweep")
                    }
            ...
            private func startScrollExperiment() {
                ...
                guard let root = NSApp.keyWindow?.contentView,
                    let table = table(in: root), let scroll = table.enclosingScrollView
                else {
        ...
        private var actions: TimelineBubbleActions {
            ...
            result.toggleReaction = { row, emoji in
                ...
                Task {
                    await fixture.reactions.toggle(message: message, emoji: emoji, currentUserID: 1)
                }
            }
    ...
    private final class TimelineBubbleFixtureModel: ObservableObject, TimelineMessageSource {
        ...
        init() {
            reactions = MessageReactionController(
                apiClient: reactionClient, messageStore: store, onInvalidToken: {})
            reactionObservation = reactions.objectWillChange.sink { [weak self] in
        ...
        func prepare() {
            ...
            do {
                let directory = FileManager.default.temporaryDirectory.appendingPathComponent(
                    "chahua-bubble-fixtures", isDirectory: true)
                ...
                for (index, size) in sizes.enumerated() {
                    ...
                    try generateImage(
                        url: url, type: UTType.png.identifier, width: size.0, height: size.1,
                        number: index)
                    media.append(
                        attachment(url: url, id: "image-\(index)", width: size.0, height: size.1))
                }
                let gif = directory.appendingPathComponent("animated.gif")
                try generateImage(
                    url: gif, type: UTType.gif.identifier, width: 240, height: 180, number: 5,
                    frames: 4)
                media.append(
                    attachment(url: gif, id: "animated", width: 240, height: 180, kind: "image/gif")
                )
                let heic = directory.appendingPathComponent("native.heic")
                try generateImage(
                    url: heic, type: UTType.heic.identifier, width: 320, height: 240, number: 6)
                media.append(
                    attachment(url: heic, id: "heic", width: 320, height: 240, kind: "image/heic"))
                var objects: [[String: Any]] = []
                let texts = [
                    "Hello", "Hello", "First line\n第二行 👨‍👩‍👧‍👦\nFinal line",
                    String(repeating: "x", count: 160),
                    "Literal *markup* and `code`; @[uid:2] @[uid:99] https://example.com/path).",
                ]
                for (index, text) in texts.enumerated() {
                    objects.append(object(index: index, text: text))
                }
                for attachment in media {
                    objects.append(
                        object(index: objects.count, text: "", attachments: [attachment]))
                }
                var unknown = media[0]
                unknown.removeValue(forKey: "width")
                unknown.removeValue(forKey: "height")
                objects.append(
                    object(
                        index: objects.count, text: "Unknown dimensions remain square",
                        attachments: [unknown]))
                let broken = attachment(
                    url: directory.appendingPathComponent("missing.png"), id: "missing", width: 200,
                    height: 150)
                objects.append(
                    object(
                        index: objects.count, text: "Missing image retains its frame",
                        attachments: [broken]))
                for count in [2, 3, 4, 5, 6, 7, 20] {
                    let attachments = (0..<count).map { index -> [String: Any] in
                        var item = media[index % media.count]
                        item["id"] = "gallery-\(count)-\(index)"
                        return item
                    }
                    var item = object(
                        index: objects.count,
                        text: "Gallery \(count): caption with reply and thread",
                        attachments: attachments)
                    item["replyToMessage"] = [
                        "id": "quoted-target", "clientGeneratedId": "quoted-client",
                        "createdAt": "2026-09-01T12:00:00Z", "sender": sender(2),
                        "messageType": "text", "attachments": [["kind": "image/png"]],
                        "mentions": [], "isDeleted": false,
                        "message": "A quoted message with enough text to exercise truncation.",
                    ]
                ...
                for deleted in [false, true] {
                    var item = object(
                        index: objects.count,
                        text: "A text reply with @[uid:2] and https://example.com")
                    item["replyToMessage"] = [
                ...
                let reactions: [[String: Any]] = [
                    [
                        ...
                        "reactors": (1...5).map { ["uid": $0, "name": "Reactor \($0)"] },
                    ],
                ...
                for outgoing in [false, true] {
                    ...
                    sticker["sticker"] = [
                        ...
                        "media": [
                            "id": "sticker-media", "url": gif.absoluteString,
                            "contentType": "image/gif", "size": 1, "width": 240, "height": 180,
                        ],
                    ]
                ...
                let video = attachment(
                    url: directory.appendingPathComponent("placeholder.mp4"),
                    id: "video-placeholder", width: 320, height: 180, kind: "video/mp4")
                objects.append(
                    object(
                        index: objects.count, text: "Video stays a placeholder",
                        attachments: [video]))
                ...
                if let count = ProcessInfo.processInfo.environment["CHAHUA_PERFORMANCE_ROWS"]
                    .flatMap(Int.init)
                {
                    ...
                    for index in objects.count..<max(objects.count, count) {
                        ...
                        for key in ["id", "clientGeneratedId", "createdAt"] {
                            ...
                            let text = item["message"] as? String, !text.isEmpty
                        {
                            item["message"] = [
                                ...
                                String(
                                    repeating:
                                        "Wrapped selectable text with different line lengths. ",
                                    count: [0, 1, 4, 12][index % 4]),
                            ].joined(separator: "\n")
                        }
                        objects.append(item)
                    }
                }
                messages = try objects.map {
                    try decoder.decode(
                        MessageResponse.self, from: JSONSerialization.data(withJSONObject: $0))
                }
                reactionClient.install(messages)
                timeline = ConversationTimelineModel(
                    chatID: "bubble-fixtures", currentUserID: 1, isGroupChat: true, source: self,
                    messageStore: store)
        ...
        func queue() async {
            ...
            store.enqueue(
                .init(
                    chatID: "bubble-fixtures", clientGeneratedID: "diagnostic-pending",
                    body: .init(
                        messageType: .text, clientGeneratedId: "diagnostic-pending",
                        message: "Pending acknowledgement retains row identity"),
        ...
        func setThreadScope(_ enabled: Bool) {
            timeline = ConversationTimelineModel(
                chatID: "bubble-fixtures", currentUserID: 1, isGroupChat: true, source: self,
                messageStore: store, threadID: enabled ? "fixture-root" : nil)
        }
        ...
        func members(matching query: ListMembersQuery) -> [MemberResponse] {
            ...
            for message in messages {
                ...
                senders[sender.uid] = MemberResponse(
                    uid: sender.uid, username: sender.name, avatarUrl: sender.avatarUrl)
            }
            let prefix = (query.q ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
                .lowercased()
            return Array(
                senders.values.filter {
            ...
            -> ListMessagesResponse
        {
            ...
            let data = try JSONSerialization.data(withJSONObject: [
                "messages": try messages.map { message in
                    var object =
                        try JSONSerialization.jsonObject(with: encoder.encode(message))
                        as! [String: Any]
                    if let threadID = query.threadID { object["replyRootId"] = threadID }
                    return object
                },
                "olderCursor": NSNull(), "newerCursor": NSNull(), "nextCursor": NSNull(),
                "prevCursor": NSNull(),
            ])
            return try decoder.decode(ListMessagesResponse.self, from: data)
        }

        private func sender(_ id: Int) -> [String: Any] {
            ["uid": id, "gender": 0, "name": id == 1 ? "Me" : "Ada"]
        }
        private func object(index: Int, text: String, attachments: [[String: Any]] = []) -> [String:
            Any]
        {
            [
                "id": "fixture-\(index)", "clientGeneratedId": "fixture-client-\(index)",
                "chatId": "bubble-fixtures", "messageType": "text",
                "sender": sender(index % 2 == 0 ? 2 : 1),
                "createdAt": ISO8601DateFormatter().string(
                    from: Date(timeIntervalSince1970: 1_788_264_000 + Double(index))),
                "isEdited": false, "isDeleted": false, "hasAttachments": !attachments.isEmpty,
                "attachments": attachments, "reactions": [],
                "mentions": [["uid": 2, "gender": 0, "username": "Ada"]], "message": text,
            ]
        }
        private func attachment(
            url: URL, id: String, width: Int, height: Int, kind: String = "image/png"
        ) -> [String: Any] {
            [
                "id": id, "url": url.absoluteString, "kind": kind, "size": 1,
                "fileName": url.lastPathComponent, "width": width, "height": height,
            ]
        }

        private func generateImage(
            url: URL, type: String, width: Int, height: Int, number: Int, frames: Int = 1
        ) throws {
            guard
            ...
            for frame in 0..<frames {
                ...
                context.setFillColor(
                    CGColor(red: 0.3 + shade * 0.5, green: 0.75 - shade * 0.4, blue: 0.65, alpha: 1)
                )
                context.fill(CGRect(x: 0, y: 0, width: width, height: height))
                let label = NSAttributedString(
                    string: "Fixture \(number) / frame \(frame)",
                    attributes: [
                        NSAttributedString.Key(kCTFontAttributeName as String):
                            CTFontCreateWithName(
                                "Helvetica" as CFString, CGFloat(min(width, height)) / 8, nil),
