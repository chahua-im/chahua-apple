...
enum DesignSystemFixtures {
    ...
    static let successfulImage = RemoteImagePhase.success(Image(systemName: "photo"))
        ...
        static var textBubbleRows: [TimelineRow] {
            ...
            for (index, state) in [PendingOutgoingMessage.State.queued, .sending, .failed]
                .enumerated()
            {
                    ...
                    .pending(
                        PendingOutgoingMessage(
                            ...
                            body: CreateMessageBody(
                                messageType: .text, clientGeneratedId: id,
                                message: "Pending: \(state.rawValue)"),
                            enqueuedAt: bubbleDate.addingTimeInterval(Double(60 + index)),
                            senderID: 1, state: state
                        )))
            }
        ...
        static var textBubbleMessages: [MessageResponse] {
            let cases: [(Int32, String?, String?)] = [
                ...
                (
                    2, "Ada",
                    String(
                        repeating: "A long message wraps naturally without hiding its timestamp. ",
                        count: 5)
                ),
                (1, "Me", "First line\nSecond line"),
                (3, "小明", "你好，世界！🌸 👨‍👩‍👧‍👦"),
                ...
                (4, nil, nil), (5, "", ""),
                (
                    6, String(repeating: "A very long sender name ", count: 8),
            ...
            return cases.enumerated().map { index, item in
                let object: [String: Any] = [
                    "id": "bubble-\(index)", "chatId": "bubble-fixtures",
                    "clientGeneratedId": "bubble-client-\(index)",
                    "messageType": index == 15 ? "image" : index == 16 ? "system" : "text",
                    "sender": [
                        "uid": item.0, "gender": 0, "name": item.1 as Any? ?? NSNull(),
                        "avatarUrl": NSNull(), "userGroup": NSNull(),
                    ],
                    "createdAt": formatter.string(
                        from: bubbleDate.addingTimeInterval(Double(index))),
                    "isEdited": false, "isDeleted": index == 14, "hasAttachments": false,
                    "attachments": [], "reactions": [], "mentions": [],
                    "message": item.2 as Any? ?? NSNull(),
                ]
                do {
        ...
        private static func bubbleRowsBuilder(isGroupChat: Bool) -> TimelineRowsBuilder {
            ...
            return TimelineRowsBuilder(
                currentUserID: 1, isGroupChat: isGroupChat, calendar: calendar)
        }
    #endif
}
