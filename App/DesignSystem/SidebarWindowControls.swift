        ...
        final class ControlHost: NSView {
            ...
            override func viewDidMoveToWindow() {
                ...
                NotificationCenter.default.addObserver(
                    self, selector: #selector(enterFullScreen),
                    name: NSWindow.didEnterFullScreenNotification, object: window)
                NotificationCenter.default.addObserver(
                    self, selector: #selector(leaveFullScreen),
            ...
            private func install() {
                ...
                for kind in [NSWindow.ButtonType.closeButton, .miniaturizeButton, .zoomButton] {
                    guard let button = window.standardWindowButton(kind),
                        let parent = button.superview, parent !== self
                    else { continue }
                    ...
                    NotificationCenter.default.removeObserver(
                        self, name: NSView.frameDidChangeNotification, object: button)
                    NotificationCenter.default.addObserver(
                        self, selector: #selector(buttonFrameDidChange),
            ...
            override func layout() {
                ...
                for placement in placements {
                    ...
                    placement.button.setFrameOrigin(
                        NSPoint(x: x, y: (bounds.height - size.height) / 2))
                    x += size.width + 8
                }
            }
            ...
            private func restore() {
                for placement in placements {
                    NotificationCenter.default.removeObserver(
                        self, name: NSView.frameDidChangeNotification, object: placement.button)
            ...
            func detach() {
                if let observedWindow {
                    NotificationCenter.default.removeObserver(
                        self, name: NSWindow.didEnterFullScreenNotification, object: observedWindow)
                    NotificationCenter.default.removeObserver(
                        self, name: NSWindow.didExitFullScreenNotification, object: observedWindow)
                    NotificationCenter.default.removeObserver(
