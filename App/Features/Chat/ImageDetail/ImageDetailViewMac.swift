...
struct ImageDetailPlatformView: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> MessageImageDetailMacView {
        MessageImageDetailMacView(gallery: gallery, mediaContext: mediaContext, onDismiss: onDismiss)
    }
...
final class MessageImageDetailMacView: NSView {
    ...
    init(gallery: MessageImageGallery, mediaContext: AppMediaContext?, onDismiss: @escaping () -> Void) {
        ...
        configureButton(closeButton, symbol: "xmark", label: String(localized: "Close image"), action: #selector(close))
        configureButton(previousButton, symbol: "chevron.left", label: String(localized: "Previous image"), action: #selector(previousImage))
        configureButton(nextButton, symbol: "chevron.right", label: String(localized: "Next image"), action: #selector(nextImage))
    ...
    private func restoreFocus() {
        ...
        if let controller = previousFirstResponder as? NSViewController,
           ...
           window.makeFirstResponder(controller) { return }
    ...
    private func refreshPages() {
        ...
        for index in first...last {
            if pages[index] == nil {
                let page = MessageImageDetailMacPage(item: gallery.items[index], mediaContext: mediaContext)
    ...
    private func updateControls() {
        ...
        countLabel.stringValue = String(localized: "\(selectedIndex + 1) of \(gallery.items.count)")
        fileNameLabel.stringValue = gallery.items[selectedIndex].fileName
        nextButton.nextKeyView = activePage?.retryButton.isHidden == false ? activePage?.retryButton : closeButton
        activePage?.retryButton.nextKeyView = closeButton
    }

    @objc private func accessibilityOptionsChanged() {
        for (index, page) in pages { page.setActive(index == selectedIndex, reduceMotion: reduceMotion) }
    }
    ...
    override func layout() {
        ...
        chrome.frame = bounds
        viewport.frame = CGRect(x: 16, y: 64, width: max(1, bounds.width - 32), height: max(1, bounds.height - 120))
        ...
        nextButton.frame = CGRect(x: max(16, bounds.width - 56), y: bounds.midY - 20, width: 40, height: 40)
        fileNameLabel.frame = CGRect(x: 32, y: max(64, bounds.height - 37), width: max(0, bounds.width - 64), height: 20)
    ...
    override func performKeyEquivalent(with event: NSEvent) -> Bool {
        guard event.window === window,
              window?.firstResponder === self
                || (window?.firstResponder as? NSView)?.isDescendant(of: self) == true else {
    ...
    override func scrollWheel(with event: NSEvent) {
        ...
        updateDrag(delta: CGPoint(x: event.scrollingDeltaX, y: event.scrollingDeltaY), timestamp: event.timestamp)
    ...
    override func mouseUp(with event: NSEvent) {
        ...
        if event.timestamp - lastTimestamp > 0.12 { velocity = .zero }
        lastMousePoint = nil
        if dragAxis == .pan { resetDrag() }
        else { finishDrag(cancelled: false) }
    }
    ...
    private func updateDrag(delta: CGPoint, timestamp: TimeInterval) {
        ...
        switch dragAxis {
        case .horizontal:
            let beyondEnd = (translation.x > 0 && selectedIndex == 0)
                || (translation.x < 0 && selectedIndex == gallery.items.count - 1)
            displayOffset = CGPoint(x: translation.x * (beyondEnd ? 0.22 : 1), y: 0)
        case .vertical:
            displayOffset = CGPoint(x: 0, y: translation.y >= 0 ? translation.y : translation.y * 0.15)
    ...
    private func finishDrag(cancelled: Bool) {
        guard acceptsInteraction else { resetDrag(); return }
        let axis = dragAxis
    ...
    private func changePage(by direction: Int) {
        guard acceptsInteraction, !isMagnifying, activePage?.isAtFit == true,
              gallery.items.indices.contains(selectedIndex + direction) else { return }
        resetDrag()
        window?.makeFirstResponder(self)
    ...
    private func dismiss(downward: Bool) {
        ...
        if reduceMotion {
            ...
            if downward { displayOffset.y = max(bounds.height, displayOffset.y + bounds.height * 0.35) }
            NSAnimationContext.runAnimationGroup({ animation in
...
private final class MessageImageDetailMacPage: NSView {
    ...
    init(item: MessageImageItem, mediaContext: AppMediaContext?) {
        ...
        subscription = Publishers.CombineLatest3(loader.$image, loader.$isLoading, loader.$failed)
    ...
    private func render(image: NSImage?, isLoading: Bool, failed: Bool) {
        ...
        statusLabel.stringValue = failed ? String(localized: "Unable to load image") : String(localized: "Loading image…")
    ...
    override func layout() {
        ...
        let imageSize = candidate.width > 0 && candidate.height > 0
            && candidate.width.isFinite && candidate.height.isFinite ? candidate : CGSize(width: 1, height: 1)
        if bounds.size != lastViewportSize || imageSize != lastImageSize {
            ...
            let scale = min(max(1, bounds.width) / imageSize.width, max(1, bounds.height) / imageSize.height)
            imageDocument.frame = CGRect(origin: .zero, size: CGSize(width: imageSize.width * scale, height: imageSize.height * scale))
            let proposed = CGPoint(
                x: center.x * imageDocument.frame.width - clip.bounds.width / 2,
                y: center.y * imageDocument.frame.height - clip.bounds.height / 2)
            clip.scroll(to: clip.constrainBoundsRect(CGRect(origin: proposed, size: clip.bounds.size)).origin)
            scrollView.reflectScrolledClipView(clip)
            lastViewportSize = bounds.size
            lastImageSize = imageSize
        }
        progress.frame = CGRect(x: bounds.midX - 16, y: bounds.midY - 48, width: 32, height: 32)
        statusLabel.frame = CGRect(x: 12, y: bounds.midY, width: max(0, bounds.width - 24), height: 22)
        retryButton.frame = CGRect(x: bounds.midX - 48, y: bounds.midY + 34, width: 96, height: 32)
    }
    ...
    func toggleZoom(at windowPoint: CGPoint, reduceMotion: Bool) {
        ...
        if reduceMotion { scrollView.setMagnification(target, centeredAt: point) }
        else {
            NSAnimationContext.runAnimationGroup { animation in
                animation.duration = 0.2
...
private final class MessageImageDetailMacClipView: NSClipView {
    ...
    override func constrainBoundsRect(_ proposedBounds: NSRect) -> NSRect {
        ...
        result.origin.x = size.width < result.width
            ? (size.width - result.width) / 2 : min(max(0, result.minX), size.width - result.width)
        result.origin.y = size.height < result.height
            ? (size.height - result.height) / 2 : min(max(0, result.minY), size.height - result.height)
        return result
    }
}
...
private final class MessageImageDetailMacDocument: NSView {
    ...
    func setAnimating(_ enabled: Bool) {
        ...
        if enabled { animatedImageView?.startAnimating() }
        else { animatedImageView?.stopAnimating() }
    }
