...
struct ImageDetailPlatformView: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: Context) -> ImageDetailViewControllerIOS {
        ImageDetailViewControllerIOS(gallery: gallery, mediaContext: mediaContext, onDismiss: onDismiss)
    }
    ...
    static func dismantleUIViewController(_ controller: ImageDetailViewControllerIOS, coordinator: ()) {
        controller.tearDown()
    }
}

@MainActor
final class ImageDetailViewControllerIOS: UIViewController, UIScrollViewDelegate, UIGestureRecognizerDelegate {
    ...
    private var canInteract: Bool {
        isVisible && applicationIsActive && !isPaging && !isTransitioning && !isDraggingToDismiss
            && !isChangingViewport && !dismissalStarted && !isTornDown
    }

    init(gallery: MessageImageGallery, mediaContext: AppMediaContext?, onDismiss: @escaping () -> Void) {
    ...
    override func viewDidLoad() {
        ...
        configureButton(closeButton, symbol: "xmark", label: String(localized: "Close image"), action: #selector(closeTapped))
        configureButton(previousButton, symbol: "chevron.left", label: String(localized: "Previous image"), action: #selector(previousTapped))
        configureButton(nextButton, symbol: "chevron.right", label: String(localized: "Next image"), action: #selector(nextTapped))
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(accessibilityChanged), name: UIAccessibility.voiceOverStatusDidChangeNotification, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(applicationResignedActive), name: UIApplication.willResignActiveNotification, object: nil)
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if UIAccessibility.isReduceMotionEnabled {
            ...
            UIView.animate(withDuration: 0.2, delay: 0, options: [.curveEaseOut], animations: show, completion: finish)
        }
    }
    ...
    override func viewDidLayoutSubviews() {
        ...
        if resized {
            ...
            pager.contentSize = CGSize(width: size.width * CGFloat(gallery.items.count), height: size.height)
            pager.setContentOffset(CGPoint(x: CGFloat(selectedIndex) * size.width, y: 0), animated: false)
            isPaging = false
        }
        ...
        countLabel.frame = CGRect(x: 64 + safe.left, y: top, width: max(0, size.width - 128 - safe.left - safe.right), height: 44)
        previousButton.frame = CGRect(x: size.width / 2 - 64, y: size.height - safe.bottom - 56, width: 48, height: 44)
        nextButton.frame = CGRect(x: size.width / 2 + 16, y: size.height - safe.bottom - 56, width: 48, height: 44)
        updatePageStates()
    }
    ...
    private func configureButton(_ button: UIButton, symbol: String, label: String, action: Selector) {
    ...
    private func updatePageWindow() {
        ...
        for index in lower...upper where pages[index] == nil {
            let page = ImageDetailPageIOS(item: gallery.items[index], mediaContext: mediaContext)
            ...
            page.frame = CGRect(x: CGFloat(index) * viewportSize.width, y: 0, width: viewportSize.width, height: viewportSize.height)
    ...
    private func updatePageStates() {
        ...
        pager.isScrollEnabled = canInteract && currentIsFit || isPaging && !isChangingViewport && !dismissalStarted
        for (index, page) in pages {
            let intersectsViewport = page.frame.intersects(pager.bounds) && viewportSize.width > 0
            let visible = isVisible && applicationIsActive && !dismissalStarted && intersectsViewport
            page.setPresentation(
                visible: visible,
                interactive: canInteract && index == selectedIndex,
                animating: visible && canInteract && index == selectedIndex && !UIAccessibility.isReduceMotionEnabled
            )
        ...
        nextButton.isEnabled = canInteract && currentIsFit && selectedIndex + 1 < gallery.items.count
    }
    ...
    private func goToPage(_ index: Int) -> Bool {
        guard canInteract, pages[selectedIndex]?.isAtFit == true,
              gallery.items.indices.contains(index), index != selectedIndex else { return false }
        isPaging = true
        updatePageStates()
        pager.setContentOffset(CGPoint(x: CGFloat(index) * viewportSize.width, y: 0), animated: !UIAccessibility.isReduceMotionEnabled)
    ...
    func scrollViewWillEndDragging(_ scrollView: UIScrollView, withVelocity velocity: CGPoint, targetContentOffset: UnsafeMutablePointer<CGPoint>) {
        ...
        let index = min(min(gallery.items.count - 1, selectedIndex + 1), max(max(0, selectedIndex - 1), proposed))
    ...
    private func finishPaging() {
        ...
        selectedIndex = min(gallery.items.count - 1, max(0, Int((pager.contentOffset.x / viewportSize.width).rounded())))
        isPaging = false
        updatePageWindow()
        updateChrome()
    ...
    func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        guard canInteract, pages[selectedIndex]?.isAtFit == true,
              !pager.isDragging, !pager.isDecelerating else { return false }
    ...
    @objc private func dragToDismiss(_ gesture: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            if distance > min(180, view.bounds.height * 0.22) || distance > 44 && velocity.y > 850 && velocity.y > abs(velocity.x) {
    ...
    private func cancelInteractiveDismissal() {
        ...
        if UIAccessibility.isReduceMotionEnabled {
            ...
            UIView.animate(withDuration: 0.3, delay: 0, usingSpringWithDamping: 0.9, initialSpringVelocity: 0, options: [.beginFromCurrentState, .curveEaseOut], animations: restore, completion: finish)
        }
    }
    ...
    private func dismissGallery(downward: Bool) {
        ...
        let hide = {
            ...
            self.chrome.alpha = 0
            self.pager.alpha = 0
            if downward { self.pager.transform = CGAffineTransform(translationX: self.pager.transform.tx, y: self.view.bounds.height) }
        }
        let finish: (Bool) -> Void = { [weak self] _ in
        ...
        if UIAccessibility.isReduceMotionEnabled {
            ...
            UIView.animate(withDuration: 0.2, delay: 0, options: [.beginFromCurrentState, .curveEaseOut], animations: hide, completion: finish)
        }
    }
}
...
private final class ImageDetailRootViewIOS: UIView {
    ...
    override func accessibilityScroll(_ direction: UIAccessibilityScrollDirection) -> Bool { onScroll?(direction) ?? false }
}
...
private final class ImageDetailPagerIOS: UIScrollView {
    ...
    override func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        if gestureRecognizer === panGestureRecognizer {
            ...
            guard canBeginPaging?() == true, abs(velocity.x) > abs(velocity.y) else { return false }
        }
    ...
    override func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        if gestureRecognizer === panGestureRecognizer {
            guard mayInteract?() == true,
                  zoomScale > minimumZoomScale + 0.01 || pinchGestureRecognizer?.state == .began || pinchGestureRecognizer?.state == .changed else { return false }
...
private final class ImageDetailPageIOS: UIView, UIScrollViewDelegate {
    ...
    init(item: MessageImageItem, mediaContext: AppMediaContext?) {
        ...
        scrollView.mayInteract = { [weak self] in self?.interactive == true && self?.installedImage != nil }
        addSubview(scrollView)
        imageCanvas.isAccessibilityElement = true
        imageCanvas.accessibilityTraits = .image
        imageCanvas.accessibilityLabel = item.fileName.isEmpty ? String(localized: "Image") : item.fileName
        ...
        observation = Publishers.CombineLatest3(loader.$image, loader.$isLoading, loader.$failed)
    ...
    func setPresentation(visible: Bool, interactive: Bool, animating: Bool) {
        ...
        if scrollView.pinchGestureRecognizer?.state != .began && scrollView.pinchGestureRecognizer?.state != .changed {
    ...
    override func layoutSubviews() {
        ...
        let labelSize = errorLabel.sizeThatFits(CGSize(width: labelWidth, height: .greatestFiniteMagnitude))
        errorLabel.frame = CGRect(x: (bounds.width - labelWidth) / 2, y: bounds.midY - labelSize.height - 12, width: labelWidth, height: labelSize.height)
        retryButton.frame = CGRect(x: bounds.midX - 60, y: bounds.midY + 8, width: 120, height: 48)
    }
    ...
    private func updateLoadingChrome() {
        ...
        if visible && installedImage == nil && !failed && loading { spinner.startAnimating() }
        else { spinner.stopAnimating() }
    }

    private func updateAnimation() {
        guard let image = installedImage, animationsAllowed,
              (image.kf.frameSource?.frameCount ?? image.kf.imageFrameCount ?? 1) > 1 else {
            animatedImage?.stopAnimating()
            animatedImage?.image = nil
    ...
    private func layoutImageAtFit() {
        ...
        let scale = min(bounds.width / max(1, imageSize.width), bounds.height / max(1, imageSize.height))
        let fitted = CGSize(width: max(1, imageSize.width * scale), height: max(1, imageSize.height * scale))
    ...
    private func centerImage() {
        ...
        imageCanvas.center = CGPoint(x: scrollView.contentSize.width / 2 + insetX, y: scrollView.contentSize.height / 2 + insetY)
    ...
    @objc private func doubleTapped(_ gesture: UITapGestureRecognizer) {
        guard mayInteract?() == true, installedImage != nil, !zoomAnimationRunning else { return }
    ...
    func viewForZooming(in scrollView: UIScrollView) -> UIView? { installedImage == nil ? nil : imageCanvas }
