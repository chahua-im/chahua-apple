...
final class TimelineLayoutCache {
    ...
    func layout(for content: TimelineRowPresentation, environment: TimelineLayoutEnvironment)
        -> TimelineRowLayout
    {
        ...
        if let layout = cachedLayout(
            for: content.row.id, key: content.layoutKey, environment: environment)
        {
            return layout
        }
        missCount += 1
        let layout = engine.layout(content, environment: environment)
        entries[content.row.id] = .init(
            key: content.layoutKey, environment: environment, layout: layout)
        return layout
    }

    func cachedLayout(
        for id: TimelineRowID, key: RowLayoutKey, environment: TimelineLayoutEnvironment
    ) -> TimelineRowLayout? {
        guard let entry = entries[id], entry.key == key, entry.environment == environment else {
            return nil
        }
        return entry.layout
    }
