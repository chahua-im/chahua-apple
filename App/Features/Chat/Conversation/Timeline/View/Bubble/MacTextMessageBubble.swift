...
enum MacBubbleMetrics {
    ...
    static func maximumBubbleWidth(rowWidth: CGFloat) -> CGFloat {
        max(0, rowWidth - 2 * rowHorizontalInset - avatarSize - avatarGap) * widthFraction
    }

    static func textOnlyHeight(layout: MacBubbleTextLayout, rowWidth: CGFloat) -> CGFloat {
...
struct MacTextMessageBubble: View {
    ...
    // same post-avatar cap used by MacBubbleRowLayout's final layout proposal.
    private var availableMediaWidth: CGFloat { MacBubbleMetrics.maximumBubbleWidth(rowWidth: context.viewportSize.width) }
    private var mediaSize: CGSize? {
        guard let first = attachments.first else { return nil }
        if attachments.count == 1 {
            return MacBubbleMediaLayout.singleSize(for: first, viewport: context.viewportSize, availableWidth: availableMediaWidth)
        }
        return MacBubbleMediaLayout.gallery(for: attachments, viewport: context.viewportSize, availableWidth: availableMediaWidth)?.size
    }

    var body: some View {
        let mediaSize = mediaSize
        MacBubbleRowLayout(isOutgoing: row.isOutgoing) {
            bubble(mediaSize: mediaSize)
            avatar
        }
        .padding(.horizontal, MacBubbleMetrics.rowHorizontalInset)
        .padding(.vertical, MacBubbleMetrics.rowVerticalInset)
    }

    private func bubble(mediaSize: CGSize?) -> some View {
        MacBubbleColumnLayout {
            ...
            if !mediaOnly {
                textContent(text: hasBody ? bodyText : "", overlay: false)
                    .padding(.horizontal, MacBubbleMetrics.textHorizontalInset)
                    .padding(.top, !attachments.isEmpty ? 4 : (row.showsSenderName || replyPreview != nil ? 0 : MacBubbleMetrics.textVerticalInset))
                    .padding(.bottom, threadCount == nil ? MacBubbleMetrics.textVerticalInset : 0)
            }
    ...
    @ViewBuilder private var avatar: some View {
        if !context.isMeasuring && (row.groupPosition == .single || row.groupPosition == .last) {
            AvatarView(
                ...
                displayName: senderName, diameter: MacBubbleMetrics.avatarSize
            )
        } else {
            Color.clear.frame(width: MacBubbleMetrics.avatarSize, height: MacBubbleMetrics.avatarSize)
        }
    }
        ...
        HStack(spacing: 8) {
            ...
            if let gender = message?.sender.gender, gender == 1 || gender == 2 {
                Text(gender == 1 ? "♂" : "♀")
                    ...
                    .foregroundStyle(macColor(hex: gender == 1 ? "3cb4f0" : "ff8080") ?? .primary)
            }
        }
    ...
    private func groupColor(_ group: UserGroupTagInfo) -> Color {
        ...
        return hex.flatMap(macColor(hex:)) ?? Color.gray.opacity(0.44)
    }
    ...
    @ViewBuilder private func reply(_ preview: MessagePreview) -> some View {
        ...
        let color = row.isOutgoing && !mediaOnly ? Color.white : macColorForUser(name: name, dark: colorScheme == .dark)
        let content = VStack(alignment: .leading, spacing: 2) {
            Text(name).font(.system(size: senderSize * 11 / 12, weight: .semibold)).opacity(0.85).lineLimit(1)
            Text(macMessagePreview(preview)).font(.system(size: senderSize)).opacity(0.7).lineLimit(1).truncationMode(.tail)
        }
...
func macColorForUser(name: String, dark: Bool) -> Color {
    ...
    return macColor(hex: palette[Int(abs(Int64(hash)) % Int64(palette.count))]) ?? .primary
}

private func macColor(hex: String) -> Color? {
