    ...
    var body: some View {
        TimelineSectionLayout(size: bubble.size, frames: frames) {
            ...
            if let frame = frames[.media] {
                Group {
                    if sticker {
                        StickerContent(sticker: row.entry.remoteMessage?.sticker, size: frame.size)
                    } else if !localAttachments.isEmpty {
                        BubbleLocalMedia(attachments: localAttachments, size: frame.size, itemFrames: layout.mediaFrames)
                    } else {
                        BubbleMedia(messageID: row.entry.remoteMessage?.id ?? "", attachments: row.entry.remoteMessage?.attachments ?? [], size: frame.size, itemFrames: layout.mediaFrames, action: actions.openMedia)
                    }
                }
                .frame(width: frame.width, height: frame.height)
