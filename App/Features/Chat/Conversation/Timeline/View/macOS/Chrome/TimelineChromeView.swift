    ...
    final class TimelineChromeView: NSView {
        ...
        private let reconciliationFailure = TimelineBannerView(
            cornerRadius: ChahuaTheme.Radius.small)
        ...
        override init(frame frameRect: NSRect) {
            ...
            for child in [
                initialLoading, initialFailure, initialBanner, olderEdge, newerEdge,
                repositionProgress, repositionFailure, reconciliationFailure, jumpControl,
            ] as [NSView] {
                child.isHidden = true
                addSubview(child)
            }
        ...
        func update(model: ConversationTimelineModel, headerInset: CGFloat, composerInset: CGFloat)
        {
            ...
            let next = Presentation(
                state: model.state, hasRows: !model.rows.isEmpty,
                showsJumpToLatest: model.showsJumpToLatest, headerInset: headerInset,
                composerInset: composerInset)
            guard presentation != next else { return }
            presentation = next
            let state = next.state
            let isRepositioning: Bool
            if case .repositioning = state.content {
        ...
        override func layout() {
            ...
            newerEdge.frame = NSRect(
                x: 0, y: max(0, height - presentation.composerInset - 48), width: width, height: 48)
            ...
            repositionProgress.frame = NSRect(
                x: max(0, (width - 52) / 2), y: max(0, (height - 52) / 2), width: 52, height: 52)
            jumpControl.frame = NSRect(
                x: max(0, width - ChahuaTheme.Spacing.large - TimelineJumpControl.diameter),
                y: max(
                    0,
                    height - presentation.composerInset - ChahuaTheme.Spacing.large
                        - TimelineJumpControl.diameter),
    ...
    @MainActor
    private func timelineChromeLabel(_ text: String = "", style: NSFont.TextStyle = .caption1)
        -> NSTextField
    {
    ...
    private final class TimelineLoadingPanel: NSView {
        ...
        init(label text: String?, material: Bool = false) {
            ...
            self.material =
                material ? timelineChromeMaterial(cornerRadius: ChahuaTheme.Radius.medium) : nil
            super.init(frame: .zero)
        ...
        override func layout() {
            ...
            progress.frame = NSRect(
                x: max(0, (bounds.width - spinnerSize) / 2), y: top, width: spinnerSize,
                height: spinnerSize)
            label?.frame = NSRect(
                x: 16, y: top + spinnerSize + 8, width: max(0, bounds.width - 32),
                height: labelHeight)
        }
    }
    ...
    private final class TimelineInitialFailurePanel: NSView {
        ...
        override func layout() {
            ...
            retry.frame = NSRect(
                x: (bounds.width - buttonWidth) / 2, y: top + 100, width: buttonWidth, height: 24)
        }
    }
        ...
        override func layout() {
            ...
            if isLoading {
                let labelWidth = min(
                    ...
                    ceil(
                        (label.stringValue as NSString).size(withAttributes: [
                ...
                label.frame = NSRect(
                    x: left + 24, y: (bounds.height - 18) / 2, width: labelWidth, height: 18)
            } else {
                let buttonWidth = min(max(0, bounds.width - 16), button.titleWidth)
                button.frame = NSRect(
                    x: max(8, bounds.width - 8 - buttonWidth), y: (bounds.height - 22) / 2,
                    width: buttonWidth, height: 22)
                label.frame = NSRect(
                    x: 8, y: (bounds.height - 18) / 2, width: max(0, button.frame.minX - 16),
                    height: 18)
            }
        }
    }
    ...
    private final class TimelineEdgeView: NSView {
        ...
        override func layout() {
            super.layout()
            progress.frame = NSRect(
                x: max(0, (bounds.width - 16) / 2), y: 16, width: 16, height: 16)
    ...
    private final class TimelineJumpControl: NSView {
        ...
        private struct TimelineJumpPreviewControl: NSViewRepresentable {
            ...
            func sizeThatFits(
                _ proposal: ProposedViewSize, nsView: TimelineJumpPreviewHost, context: Context
            ) -> CGSize? {
