...
final class TimelineViewController: NSViewController {
    ...
    override func viewDidLoad() {
        ...
        notifications.addObserver(self, selector: #selector(windowGeometryChanged(_:)), name: NSWindow.didMoveNotification, object: nil)
        notifications.addObserver(self, selector: #selector(windowGeometryChanged(_:)), name: NSWindow.didResizeNotification, object: nil)
        notifications.addObserver(self, selector: #selector(windowGeometryChanged(_:)), name: NSWindow.didChangeBackingPropertiesNotification, object: nil)
        refreshSurface()
    }
    ...
    private func applyAppearance() {
        ...
        view.layer?.backgroundColor = NSColor(ChahuaTheme.conversationBackground(for: colorScheme)).cgColor
    }
    ...
    @objc private func windowGeometryChanged(_ notification: Notification) {
        guard isViewLoaded, let window = notification.object as? NSWindow, window === view.window else { return }
        requestRefresh()
    }
