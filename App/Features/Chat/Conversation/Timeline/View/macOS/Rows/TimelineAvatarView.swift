...
final class TimelineAvatarView: NSView {
    ...
    func configure(url: URL?, name: String, userID: Int32?, diameter: CGFloat, displayScale: CGFloat,
        ...
        if self.name != name || self.diameter != diameter {
            ...
            initials = NSAttributedString(string: String(name.prefix(2)).uppercased(), attributes: [
            ...
            fallbackColor = NSColor(calibratedHue: (hue < 0 ? hue + 360 : hue) / 360,
                                    saturation: 0.55 / 0.775, brightness: 0.775, alpha: 1).cgColor
        ...
        imageView.configure(url: url, contentMode: .fill, animates: false, showsBlurredBackdrop: false,
                            thumbnailPixelSize: CGSize(width: pixels, height: pixels), mediaContext: mediaContext)
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        initials?.draw(in: CGRect(x: 0, y: (bounds.height - initialsLineHeight) / 2,
