    ...
    final class TimelineReactionsView: NSView {
        ...
        func bind(_ binding: TimelineRowBinding) {
            ...
            let reactions = (row.entry.remoteMessage?.reactions ?? []).sorted(
                by: TimelineRowPresentation.reactionOrder)
            ...
            let pending = binding.actions.pendingReactionMessageIDs.contains(
                row.entry.remoteMessage?.id ?? "")
            let eligibility = MessageReactionEligibility(
                canReact: MessageActionPolicy(row: row, context: binding.actions.interactionContext)
                    .canReact && binding.actions.toggleReaction != nil,
            ...
            for index in pills.indices {
                ...
                pill.isHidden = false
                pill.configure(
                    reaction, outgoing: row.isOutgoing, pending: pending,
                    enabled: eligibility.canToggle(reaction.emoji),
                    displayScale: binding.presentation.environment.displayScale,
                    mediaContext: binding.mediaContext,
                    contentFrames: index < binding.layout.reactionContentFrames.count
                        ? binding.layout.reactionContentFrames[index] : [])
        ...
        override func layout() {
            ...
            for index in pills.indices {
                pills[index].frame = index < frames.count ? frames[index] : .zero
            }
        }
        ...
        private func activate(_ emoji: String) {
            ...
            let eligibility = MessageReactionEligibility(
                canReact: MessageActionPolicy(row: row, context: binding.actions.interactionContext)
                    .canReact,
                isReacting: binding.actions.pendingReactionMessageIDs.contains(message.id),
    ...
    private final class ReactionLabel: NSTextField {
        init() {
            ...
            cell = ReactionLabelCell(textCell: "")
            isEditable = false
            isSelectable = false
            isBordered = false
    ...
    private final class TimelineReactionButton: NSButton {
        ...
        func configure(
            ...
            while avatars.count < avatarCount {
                ...
                if let last = avatars.last {
                    addSubview(avatar, positioned: .below, relativeTo: last)
                } else {
                    addSubview(avatar)
                }
            ...
            for index in avatars.indices {
                ...
                avatar.configure(
                    url: reactor.avatarUrl.flatMap(URL.init(string:)),
                    name: reactor.name ?? "User \(reactor.uid)",
                    userID: reactor.uid, diameter: 23, displayScale: displayScale,
                    mediaContext: mediaContext)
                avatar.setAccessibilityElement(false)
                avatar.setVisible(visible)
            }
            if avatarCount > 0, reaction.count > 5 {
                countLabel.stringValue = "+\(reaction.count - 5)"
                countLabel.font = .systemFont(ofSize: 11)
            } else if avatarCount == 0, reaction.count > 1 {
                countLabel.stringValue = "\(reaction.count)"
        ...
        override func layout() {
            ...
            for index in avatars.indices {
                avatars[index].frame =
                    index < avatarCount && index + 2 < contentFrames.count
                    ? contentFrames[index + 2] : .zero
            }
        }
        ...
        private func updatePaint() {
            effectiveAppearance.performAsCurrentDrawingAppearance {
                ...
                let foreground =
                    selected
                    ? NSColor.white
                    : NSColor(ChahuaTheme.ChatBubble.incomingForeground(for: dark ? .dark : .light))
                let background: NSColor
                if outgoing && selected {
                    background = NSColor(
                        srgbRed: 38 / 255, green: 107 / 255, blue: 180 / 255, alpha: 1)
                } else if selected {
                    background = NSColor(
                        srgbRed: 64 / 255, green: 135 / 255, blue: 210 / 255, alpha: 1)
                } else if dark {
                    background = NSColor(
                        srgbRed: 30 / 255, green: 32 / 255, blue: 35 / 255, alpha: 1)
                } else {
                    background = NSColor(
                        srgbRed: 215 / 255, green: 216 / 255, blue: 218 / 255, alpha: 1)
                }
