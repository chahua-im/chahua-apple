...
final class TimelineRowView: NSView {
    ...
    override init(frame frameRect: NSRect) {
        ...
        highlightLayer.actions = ["bounds": NSNull(), "position": NSNull(), "backgroundColor": NSNull(), "opacity": NSNull()]
    ...
    func bind(_ binding: TimelineRowBinding) {
        ...
        case .message(let row):
            ...
            else { bindMessage(binding, row: row, resetSelection: resetSelection) }
        }
        if !canReply { resetHover() }
        updateReplyVisibility()
        updateHighlight(animated: !resetSelection && previous?.context.isHighlighted != binding.context.isHighlighted)
    ...
    override func viewDidMoveToWindow() {
        ...
        else { setVisible(!isHiddenOrHasHiddenAncestor && !visibleRect.isEmpty) }
    }
    ...
    override func updateTrackingAreas() {
        ...
        guard !rect.isEmpty else { rowTrackingArea = nil; return }
        let area = NSTrackingArea(rect: rect, options: [.mouseEnteredAndExited, .activeInKeyWindow], owner: self)
        addTrackingArea(area); rowTrackingArea = area
    }

    override func mouseEntered(with event: NSEvent) { updateHover() }
    override func mouseExited(with event: NSEvent) { updateHover() }
    override func viewDidChangeEffectiveAppearance() { super.viewDidChangeEffectiveAppearance(); updateHighlight(animated: false) }
    ...
    private func bindMessage(_ binding: TimelineRowBinding, row: TimelineMessageRow, resetSelection: Bool) {
        ...
        if binding.layout.frames[.bubble] != nil {
            ...
            if let bubbleView { view = bubbleView }
            else { view = TimelineBubbleContentView(frame: .zero); addSubview(view); bubbleView = view }
            view.isHidden = false; view.bind(binding, resetSelection: resetSelection); view.setVisible(visible)
        } else { bubbleView?.clear(); bubbleView?.isHidden = true }
        if !preview, binding.layout.frames[.avatar] != nil {
            let view: TimelineAvatarView
            if let avatarView { view = avatarView }
            else { view = TimelineAvatarView(frame: .zero); addSubview(view); avatarView = view }
            ...
            view.isHidden = false
            view.configure(url: url, name: name, userID: row.entry.senderID, diameter: binding.presentation.environment.avatarSize,
                           displayScale: binding.presentation.environment.displayScale, mediaContext: binding.mediaContext)
            view.setVisible(visible)
        } else { avatarView?.clear(); avatarView?.isHidden = true }
        if !preview, binding.layout.frames[.reactions] != nil {
            let view: TimelineReactionsView
            if let reactionsView { view = reactionsView }
    ...
    private var canReply: Bool {
        guard let binding, !binding.context.isInteractionPreview, case .message(let row) = binding.presentation.row,
              ...
              binding.actions.replyToMessage != nil else { return false }
    ...
    private func updateHover() {
        ...
        let hovered = visible && window?.isKeyWindow == true
            && window.map { bounds.intersection(visibleRect).contains(convert($0.mouseLocationOutsideOfEventStream, from: nil)) } == true
            && canReply
        if !hovered { resetHover() }
        else { isRowHovered = true }
        updateReplyVisibility()
    }
    private func resetHover() { isRowHovered = false; replyButton?.resetHover() }
    private func updateReplyVisibility() {
        guard visible, isRowHovered, canReply else { replyButton?.isHidden = true; return }
        if replyButton == nil {
            let button = TimelineHoverReplyButton(frame: .zero)
            button.target = self; button.action = #selector(reply)
            addSubview(button); replyButton = button; needsLayout = true
        }
        replyButton?.isHidden = false
    }
    @objc private func reply() {
        guard canReply, let binding, case .message(let row) = binding.presentation.row,
              let message = row.entry.remoteMessage else { return }
        binding.actions.replyToMessage?(message)
    }
    private func updateHighlight(animated: Bool) {
        let highlighted = binding?.context.isHighlighted == true && binding?.context.isInteractionPreview != true
        let opacity: Float = highlighted ? 1 : 0
        if animated {
            let animation = CABasicAnimation(keyPath: "opacity")
            animation.fromValue = highlightLayer.presentation()?.opacity ?? highlightLayer.opacity
            animation.toValue = opacity; animation.duration = 0.3
            animation.timingFunction = CAMediaTimingFunction(name: .easeOut)
            highlightLayer.add(animation, forKey: "highlight")
        } else { highlightLayer.removeAnimation(forKey: "highlight") }
        highlightLayer.opacity = opacity
        effectiveAppearance.performAsCurrentDrawingAppearance {
            highlightLayer.backgroundColor = NSColor(ChahuaTheme.accent).withAlphaComponent(0.15).cgColor
        }
    }
    private func updateAccessibilityActions() {
        guard let binding, !binding.context.isInteractionPreview, binding.actions.openContextMenu != nil,
              case .message(let row) = binding.presentation.row, row.entry.messageType != .system,
              binding.layout.frames[.bubble] != nil else {
            setAccessibilityCustomActions(nil)
            setAccessibilityElement(false)
            return
        }
        ...
        setAccessibilityCustomActions([NSAccessibilityCustomAction(name: String(localized: "Message actions"), handler: { [weak self] in
...
private final class TimelineHoverReplyButton: NSButton {
    ...
    override init(frame frameRect: NSRect) {
        super.init(frame: frameRect)
        title = ""; isBordered = false; imagePosition = .imageOnly; imageScaling = .scaleNone
        setButtonType(.momentaryPushIn); wantsLayer = true; layer?.cornerRadius = 14
        setAccessibilityLabel(String(localized: "Reply")); toolTip = String(localized: "Reply")
        updatePaint()
    }
    required init?(coder: NSCoder) { fatalError("init(coder:) has not been implemented") }
    func resetHover() { if hovered { hovered = false; updatePaint() } }
    override func updateTrackingAreas() {
        super.updateTrackingAreas()
    ...
    override func mouseEntered(with event: NSEvent) { hovered = true; updatePaint() }
    ...
    private func updatePaint() {
        effectiveAppearance.performAsCurrentDrawingAppearance {
            ...
            image = NSImage(systemSymbolName: "arrowshape.turn.up.left", accessibilityDescription: nil)?.withSymbolConfiguration(.init(pointSize: 16, weight: .regular).applying(.init(paletteColors: [color])))
...
private final class TimelineStandaloneView: NSView {
    ...
    func clear() { presentation = nil; attributed = NSAttributedString(string: ""); setAccessibilityLabel(nil); needsDisplay = true }
    override func layout() { super.layout(); layer?.cornerRadius = isDate ? bounds.height / 2 : 0 }
    override func viewDidChangeEffectiveAppearance() { super.viewDidChangeEffectiveAppearance(); updatePaint() }
    private func updatePaint() {
        ...
        effectiveAppearance.performAsCurrentDrawingAppearance {
            ...
            case .dateSeparator:
                isDate = true
                attributed = NSAttributedString(string: text, attributes: [.font: NSFont.systemFont(ofSize: p.environment.captionSize), .foregroundColor: NSColor(ChahuaTheme.secondaryText), .paragraphStyle: paragraph])
                layer?.backgroundColor = NSColor(ChahuaTheme.secondaryBackground).cgColor
            case .unreadSeparator:
                attributed = NSAttributedString(string: text, attributes: [.font: NSFont.systemFont(ofSize: p.environment.captionSize), .foregroundColor: NSColor(ChahuaTheme.ChatBubble.outgoingBackground), .paragraphStyle: paragraph])
            case .message(let row):
                paragraph.lineSpacing = 3
                let dark = effectiveAppearance.bestMatch(from: [.darkAqua, .aqua]) == .darkAqua
                let color = dark ? NSColor(srgbRed: 152 / 255, green: 154 / 255, blue: 162 / 255, alpha: 1) : NSColor(srgbRed: 99 / 255, green: 100 / 255, blue: 105 / 255, alpha: 1)
                let result = NSMutableAttributedString(string: "")
                if let name = row.entry.remoteMessage?.sender.name, !name.isEmpty {
                    result.append(NSAttributedString(string: name + " ", attributes: [.font: NSFont.systemFont(ofSize: 13, weight: .semibold), .foregroundColor: color, .paragraphStyle: paragraph]))
                }
                result.append(NSAttributedString(string: text, attributes: [.font: NSFont.systemFont(ofSize: 13), .foregroundColor: color, .paragraphStyle: paragraph]))
                attributed = result
            }
