    ...
    private final class TimelineMediaTileView: NSButton {
        ...
        override func hitTest(_ point: NSPoint) -> NSView? {
            guard !isHidden, bounds.contains(convert(point, from: superview)), isEnabled else {
                return nil
            }
            return self
        }
        ...
        private func configureSurface(gallery: Bool, video: Bool, overflow: Int) {
            ...
            surface.layer?.backgroundColor =
                gallery && !video
                ? NSColor(srgbRed: 244.0 / 255, green: 244.0 / 255, blue: 245.0 / 255, alpha: 1)
                    .cgColor
                : NSColor.black.cgColor
            if (overflowCount > 0) != (overflow > 0) {
                surface.layer?.filters =
                    overflow > 0
                    ? [CIFilter(name: "CIGaussianBlur", parameters: [kCIInputRadiusKey: 2])]
                        .compactMap { $0 }
                    : nil
            }
