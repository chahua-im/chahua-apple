...
final class TimelineBubbleContentView: UIView {
    ...
    func bind(_ binding: TimelineRowBinding, resetSelection: Bool) {
        ...
        if localFrames[.title] != nil, let title = presentation.title {
            ...
            view.isHidden = false
            view.configure(title: title, row: row, fontSize: environment.captionSize, frames: binding.layout.titleFrames)
        } else {
            headerView?.clear()
        ...
        if localFrames[.reply] != nil, let reply = presentation.reply {
            ...
            view.isHidden = false
            view.configure(reply, outgoing: row.isOutgoing, filled: isFilled, fontSize: environment.captionSize,
                           frames: binding.layout.replyContentFrames, enabled: binding.actions.openReply != nil)
        } else {
            replyView?.clear()
        ...
        if localFrames[.text] != nil, let geometry = binding.layout.textGeometry {
            ...
            view.apply(MessageTextContent(
                ...
                failureAction: failureAction, geometry: geometry, fontSize: environment.bodySize
        ...
        if localFrames[.metadata] != nil, let metadata = presentation.metadata {
            ...
            view.isHidden = false
            view.configure(metadata, overlay: presentation.metadataIsOverlay && localFrames[.media] != nil,
    ...
    override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        guard !isHidden, alpha > 0, isUserInteractionEnabled, self.point(inside: point, with: event) else { return nil }
        ...
        if let metadataView, !metadataView.isHidden,
           let target = metadataView.hitTestFailureButton(metadataView.convert(point, from: self), with: event) {
            return target
        }
    ...
    private func updateSurface() {
        ...
        let color = row.isOutgoing ? ChahuaTheme.ChatBubble.outgoingBackground : ChahuaTheme.ChatBubble.incomingBackground(for: dark ? .dark : .light)
    ...
    private func makeHeader() -> TimelineHeaderView {
        ...
        sections.addSubview(view); headerView = view
        return view
    }
    ...
    private func makeReply() -> TimelineReplyView {
        ...
        view.onActivate = { [weak self] in
            guard let binding = self?.binding, let reply = binding.presentation.reply else { return }
            binding.actions.openReply?(reply.id)
        }
        sections.addSubview(view); replyView = view
        return view
    }
...
private final class TimelineHeaderView: UIView {
    ...
    func configure(title: TitleContent, row: TimelineMessageRow, fontSize: CGFloat, frames: [CGRect]) {
        ...
        name.text = title.name; name.font = .systemFont(ofSize: fontSize, weight: .semibold)
        group.text = title.groupName ?? ""; group.font = .systemFont(ofSize: fontSize)
        gender.text = title.genderGlyph ?? ""; gender.font = .systemFont(ofSize: fontSize)
        ...
        accessibilityLabel = [title.name, title.groupName, title.genderGlyph].compactMap { $0 }.joined(separator: " ")
        updateColors()
        setNeedsLayout()
    }
    ...
    func clear() {
        ...
        name.text = nil; group.text = nil; gender.text = nil
    ...
    private func updateColors() {
        ...
        groupSurface.backgroundColor = resolved.withAlphaComponent(resolved.cgColor.alpha * 0.85).cgColor
        gender.textColor = title.genderGlyph.flatMap { bubbleColor(hex: $0 == "♂" ? "3cb4f0" : "ff8080") }.map { UIColor($0) } ?? .label
    }
}
...
private final class TimelineReplyView: UIControl {
    ...
    func configure(_ preview: MessagePreview, outgoing: Bool, filled: Bool, fontSize: CGFloat, frames: [CGRect], enabled: Bool) {
        self.preview = preview; self.outgoing = outgoing; self.filled = filled
        contentFrames = frames
        author.text = preview.sender.name.flatMap { $0.isEmpty ? nil : $0 } ?? "User \(preview.sender.uid)"
    ...
    func clear() {
        ...
        author.text = nil; previewLabel.text = nil
        isEnabled = false
        accessibilityLabel = nil
        marker.stop()
    }
    ...
    private func updateColors() {
        ...
        let dark = traitCollection.userInterfaceStyle == .dark
        let color = outgoing && filled ? UIColor.white : UIColor(bubbleColorForUser(uid: preview.sender.uid, dark: dark))
        ...
        stripe.backgroundColor = color.withAlphaComponent(outgoing ? 0.5 : 1).resolvedColor(with: traitCollection).cgColor
    }
}
...
private final class TimelineMetadataView: UIView {
    ...
    func clear() {
        metadata = nil; failureAction = nil
        failureButton?.clear(); failureButton?.isHidden = true
        accessibilityLabel = nil
        accessibilityHint = nil
...
private final class BubbleStandaloneView: UIView {
    ...
    func configure(text: String, deleted: Bool, outgoing: Bool, fontSize: CGFloat, symbolSize: CGSize, gap: CGFloat) {
        self.text = text; self.deleted = deleted; self.outgoing = outgoing
        self.fontSize = fontSize; self.symbolSize = symbolSize; self.gap = gap
    ...
    private func updatePaint() {
        let dark = traitCollection.userInterfaceStyle == .dark
        let foreground = UIColor(outgoing ? ChahuaTheme.ChatBubble.outgoingForeground : ChahuaTheme.ChatBubble.incomingForeground(for: dark ? .dark : .light))
        let font = deleted ? UIFont.italicSystemFont(ofSize: fontSize) : UIFont.systemFont(ofSize: fontSize)
        ...
        symbol = deleted ? nil : UIImage(systemName: "questionmark.square.dashed", withConfiguration: configuration)
        setNeedsDisplay()
    }
    ...
    override func draw(_ rect: CGRect) {
        ...
        attributed.draw(with: CGRect(x: offset, y: 0, width: max(0, bounds.width - offset), height: bounds.height),
...
private func bubblePath(in rect: CGRect, outgoing: Bool, hasTail: Bool, drawsTail: Bool) -> CGPath {
    ...
    tail.addArc(center: center, radius: 1, startAngle: atan2(15.3 - center.y, 1.3 - center.x),
                endAngle: atan2(17 - center.y, 1 - center.x), clockwise: true)
    tail.closeSubpath()
    let transform = outgoing
