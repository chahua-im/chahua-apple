...
final class TimelineMediaView: UIView {
    ...
    func bind(_ binding: TimelineRowBinding) {
        ...
        if row.entry.messageType == .sticker {
            ...
            itemFrames = count == 1 ? [CGRect(origin: .zero, size: mediaFrame.size)]
    ...
    override func layoutSubviews() {
        ...
        for (index, frame) in itemFrames.enumerated() where index < tiles.count { tiles[index].frame = frame }
    }
...
private final class TimelineMediaTileView: UIControl {
    ...
    private let stickerEmoji = UILabel(frame: .zero)
    ...
    func configureSticker(_ sticker: MessageStickerResponse?, displayScale: CGFloat, captionSize: CGFloat,
        ...
        if let sticker, sticker.media.contentType.hasPrefix("image/") {
            ...
            stickerEmoji.text = sticker?.emoji
            stickerEmoji.isHidden = sticker?.emoji.isEmpty != false
            showWarning(symbol: "photo.badge.exclamationmark", label: String(localized: "Sticker preview unavailable"), color: .secondaryLabel)
        }
        isEnabled = false
        onOpen = nil
    ...
    override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        guard isEnabled, !isHidden, alpha > 0, self.point(inside: point, with: event) else { return nil }
        return self
    }
    ...
    private func updateTapMarker() {
        if isEnabled, onOpen != nil { tapMarker.configure(.tap { [weak self] in self?.activate() }) }
        else { tapMarker.stop() }
    }
    ...
    private func configureSurface(gallery: Bool, video: Bool, overflow: Int) {
        ...
        surface.backgroundColor = gallery && !video
    ...
    private func startOverflowBlurIfNeeded() {
        ...
        if let overflowBlur { blur = overflowBlur }
        else {
    ...
    private func startLocalIfNeeded() {
        ...
        localCompletion = Task { [weak self] in
            ...
            guard !Task.isCancelled, let self, self.localGeneration == current,
                  self.localPath == path, self.active else { return }
