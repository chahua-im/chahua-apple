...
struct TimelineRowRepresentable: UIViewRepresentable {
    ...
    private var size: CGSize { context.isInteractionPreview ? layout.frames[.bubble]?.size ?? layout.size : layout.size }
    ...
    func sizeThatFits(_ proposal: ProposedViewSize, uiView: TimelineRowView, context: Context) -> CGSize? { size }
