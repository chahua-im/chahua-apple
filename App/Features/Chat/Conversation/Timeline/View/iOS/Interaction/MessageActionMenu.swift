        ...
        private var actionGrid: some View {
            ...
            return VStack(spacing: 0) {
                ForEach(0..<rowCount, id: \.self) { rowIndex in
                    ...
                    HStack(spacing: 0) {
                        ForEach(
                            menuActions[(rowIndex * 5)..<min((rowIndex + 1) * 5, menuActions.count)]
                        ) { action in
                            actionButton(action)
        ...
        var body: some View {
            shape.fill(.regularMaterial)
                .overlay {
                    shape.fill(
                        colorScheme == .dark
                            ? Color(white: 0.12).opacity(0.65) : Color.white.opacity(0.65))
                }
        }
    }
    ...
    private struct MessageReactionButtonStyle: ButtonStyle {
        func makeBody(configuration: Configuration) -> some View {
            configuration.label
                .background(
                    configuration.isPressed ? Color.primary.opacity(0.08) : .clear, in: Circle())
        }
    }
