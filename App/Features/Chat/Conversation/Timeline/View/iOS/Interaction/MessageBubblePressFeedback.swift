...
final class MessageBubblePressFeedback {
    ...
    init(view: UIView) {
        ...
        let animator = UIViewPropertyAnimator(duration: reduceMotion ? 0.12 : 0.38, curve: .easeOut) { [weak view] in
            view?.transform = reduceMotion ? .identity : CGAffineTransform(scaleX: 0.975, y: 0.975)
    ...
    func restore(animated: Bool = false) {
        ...
        if animated, phase != .hidden, view.window != nil {
            phase = .restoring
            let animator = UIViewPropertyAnimator(duration: 0.16, curve: .easeOut) { [weak view] in
                view?.transform = .identity
                view?.alpha = 1
            }
            self.animator = animator
