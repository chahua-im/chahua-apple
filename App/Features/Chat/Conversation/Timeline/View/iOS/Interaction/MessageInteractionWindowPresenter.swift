...
struct MessageInteractionWindowPresenter: UIViewRepresentable {
    ...
    static func dismantleUIView(_ view: MessageInteractionWindowAnchor, coordinator: Coordinator) {
        ...
        func attach(to window: UIWindow?) {
            ...
            lift = Task { @MainActor [weak self, weak controller] in
                ...
                guard let self, let controller, self.controller === controller,
                    self.isPresented, !Task.isCancelled else { return }
                let entry: Animation = self.reduceMotion
