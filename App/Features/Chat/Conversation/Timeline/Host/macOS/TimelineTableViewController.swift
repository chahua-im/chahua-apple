...
final class TimelineTableViewController: NSViewController, NSTableViewDataSource, NSTableViewDelegate {
    ...
    private func captureAnchor() -> (TimelineRowID, CGFloat)? { let visible = scrollView.documentVisibleRect; let range = tableView.rows(in: visible); guard range.location != NSNotFound, range.length > 0 else { return nil }; return (rows[range.location].id, tableView.rect(ofRow: range.location).minY - visible.minY) }
