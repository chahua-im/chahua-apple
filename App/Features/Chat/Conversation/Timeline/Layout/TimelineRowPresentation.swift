    ...
    static func make(row: TimelineRow, currentUserProfile: MeResponse?, currentUserID: Int32?, isThreadTimeline: Bool, environment: TimelineLayoutEnvironment) -> Self {
        ...
        case .message(let message):
            ...
            if system {
                ...
                title = message.showsSenderName && !sticker ? .init(name: name, groupName: sender?.userGroup?.name.flatMap { $0.isEmpty ? nil : $0 }, genderGlyph: sender?.gender == 1 ? "♂" : sender?.gender == 2 ? "♀" : nil) : nil
