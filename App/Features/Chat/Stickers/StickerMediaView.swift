...
struct StickerMediaView: View {
    ...
    var body: some View {
        StickerMediaRepresentable(media: media, emoji: emoji, mediaContext: mediaContext,
...
private struct StickerMediaRepresentable: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> StickerMediaSurfaceView { StickerMediaSurfaceView(frame: .zero) }
    func updateUIView(_ view: StickerMediaSurfaceView, context: Context) {
        view.configure(media: media, emoji: emoji, displayScale: displayScale, mediaContext: mediaContext)
        view.setVisible(visible)
    }
    static func dismantleUIView(_ view: StickerMediaSurfaceView, coordinator: ()) { view.clear() }
}
#else
private struct StickerMediaRepresentable: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> StickerMediaSurfaceView { StickerMediaSurfaceView(frame: .zero) }
    func updateNSView(_ view: StickerMediaSurfaceView, context: Context) {
        view.configure(media: media, emoji: emoji, displayScale: displayScale, mediaContext: mediaContext)
        view.setVisible(visible)
    }
    static func dismantleNSView(_ view: StickerMediaSurfaceView, coordinator: ()) { view.clear() }
}
#endif
...
final class StickerMediaSurfaceView: StickerNativeView {
    ...
    func configure(media: MessageStickerMediaResponse, emoji: String, displayScale: CGFloat, mediaContext: AppMediaContext?) {
        ...
        isVideo = type.hasPrefix("video/") || URL(string: media.url)?.pathExtension.lowercased() == "webm"
        imageView.isHidden = failure != nil || (isVideo && (!hasVideoPoster || !videoLoading))
        if changed {
            guard let url = URL(string: media.url), ["https", "http"].contains(url.scheme?.lowercased() ?? ""), url.host != nil else {
            ...
            if isVideo {
                ...
                playback.onStatus = { [weak self] error, loading in
                    ...
                    if let error { self.showFailure(error) }
                    else {
                ...
                showFailure(String(localized: "Unsupported sticker media type: \(media.contentType)"))
                return
            }
        }
        layoutMedia()
    ...
    #if os(iOS)
    override func layoutSubviews() { super.layoutSubviews(); layoutMedia(); refreshVisibility() }
    override func didMoveToWindow() { super.didMoveToWindow(); observeScrolling(); refreshVisibility() }
    override func didMoveToSuperview() { super.didMoveToSuperview(); observeScrolling(); refreshVisibility() }
    override var isHidden: Bool { didSet { refreshVisibility() } }
    override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? { nil }
    #else
    override func layout() { super.layout(); layoutMedia(); refreshVisibility() }
    override func viewDidMoveToWindow() { super.viewDidMoveToWindow(); observeScrolling(); refreshVisibility() }
    override func viewDidMoveToSuperview() { super.viewDidMoveToSuperview(); observeScrolling(); refreshVisibility() }
    ...
    @objc private func visibilityChanged(_ notification: Notification) {
        ...
        if notification.name == UIApplication.willResignActiveNotification || notification.name == UIScene.willDeactivateNotification {
    ...
    private func refreshVisibility() {
        #if os(iOS)
        let inViewport = observedScrollView.map { convert(bounds, to: $0).intersects($0.bounds) } ?? true
        let visible = requestedVisible && timelineMediaIsVisible && inViewport
        #else
        let visible = requestedVisible && window?.isVisible == true && window?.isMiniaturized == false
            && window?.occlusionState.contains(.visible) == true && !isHiddenOrHasHiddenAncestor && !visibleRect.isEmpty
            && NSApp.isActive
        #endif
    ...
    private func layoutMedia() {
        ...
        statusLabel.frame = CGRect(x: 4, y: (bounds.height - height) / 2, width: max(0, bounds.width - 8), height: height)
        guard let media, !isVideo, failure == nil, bounds.width > 0, bounds.height > 0 else { return }
        let pixels = CGSize(width: ceil(bounds.width * displayScale), height: ceil(bounds.height * displayScale))
        guard pixels != imagePixels else { return }
        imagePixels = pixels
        imageView.onLoadFailure = { [weak self] error in self?.showFailure(error) }
        imageView.configure(url: URL(string: media.url), contentMode: .fit,
    ...
    private static func posterKey(_ url: URL) -> String { "sticker-video-poster:" + url.absoluteString }
    ...
    private func installVideoPoster(_ image: StickerNativeImage) {
        ...
        hasVideoPoster = true
        imageView.configureLocal(image: pixels, contentMode: .fit, showsBlurredBackdrop: false, isLoading: false)
    ...
    private func updateAccessibility() {
        let label = emoji.isEmpty ? String(localized: "Sticker") : String(localized: "Sticker \(emoji)")
        #if os(iOS)
        accessibilityLabel = label
        accessibilityValue = failure ?? (videoLoading && !hasVideoPoster ? String(localized: "Loading sticker") : nil)
        #else
        setAccessibilityLabel(label)
        setAccessibilityValue(failure ?? (videoLoading && !hasVideoPoster ? String(localized: "Loading sticker") : nil))
        #endif
    }
}
...
private final class StickerVideoPlayback: NSObject, WKScriptMessageHandler, WKNavigationDelegate {
    ...
    init(url: URL, contentType: String, dataStore: WKWebsiteDataStore) {
        ...
        configuration.userContentController.add(StickerVideoMessageRelay(owner: self), name: "sticker")
    ...
    func setVisible(_ visible: Bool) {
        ...
        if visible { start() }
        else { view.evaluateJavaScript("window.stickerSetVisible?.(false)") }
    }
    ...
    func stop() {
        ...
        view.evaluateJavaScript("(() => { const v = document.querySelector('video'); if (v) { v.pause(); v.removeAttribute('src'); v.load(); } })()")
        view.stopLoading()
    ...
    fileprivate func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        guard loaded, let body = message.body as? [String: String], body["token"] == token else { return }
        switch body["status"] {
        case "ready":
            onStatus?(nil, false)
            capturePoster()
        case "unsupported", "error:4": onStatus?(String(localized: "This device cannot decode this sticker’s video format."), false)
        case "error:2": onStatus?(String(localized: "Sticker video could not be downloaded."), false)
        case "error:3": onStatus?(String(localized: "Sticker video is damaged or uses an unsupported codec."), false)
